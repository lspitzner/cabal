...
--
brokenPackages
  :: (PackageFixedDeps pkg) => PackageIndex pkg -> [(pkg, [UnitId])]
brokenPackages index =
  [ (pkg, missing)
  | pkg <- allPackages index
  , let missing =
          [ pkg'
          | pkg' <- CD.flatDeps (depends pkg)
...
-- for them here.
dependencyInconsistencies
  :: forall pkg . (PackageFixedDeps pkg, HasUnitId pkg) => IndependentGoals
     -> PackageIndex pkg
...
dependencyInconsistencies indepGoals index = concatMap
  ...
  where
    ...
    subplans =
      rights $ map (dependencyClosure index) (rootSets indepGoals index)
...
libraryRoots index = map toPkgId roots
  where
...
setupRoots = filter (not . null) . map (CD.setupDeps . depends) . allPackages
...
--
dependencyInconsistencies'
  :: forall pkg . (PackageFixedDeps pkg, HasUnitId pkg) => PackageIndex pkg
     -> [(PackageName, [(PackageIdentifier, Version)])]
dependencyInconsistencies' index =
  ...
  where
    ...
    reallyIsInconsistent [p1, p2] =
      ...
      in  pid1
            `notElem` CD.nonSetupDeps (depends p2)
            &&        pid2
            `notElem` CD.nonSetupDeps (depends p1)
...
--
dependencyCycles
  :: (PackageFixedDeps pkg, HasUnitId pkg) => PackageIndex pkg -> [[pkg]]
dependencyCycles index =
  [ vs | Graph.CyclicSCC vs <- Graph.stronglyConnComp adjacencyList ]
  where
    adjacencyList =
      [ (pkg, installedUnitId pkg, CD.flatDeps (depends pkg))
      | pkg <- allPackages index
      ]
...
dependencyClosure
  ...
  => PackageIndex pkg
  -> [UnitId]
  -> Either [(pkg, [UnitId])] (PackageIndex pkg)
...
dependencyGraph
  :: (PackageFixedDeps pkg, HasUnitId pkg)
  => PackageIndex pkg
  -> (Graph.Graph, Graph.Vertex -> UnitId, UnitId -> Maybe Graph.Vertex)
dependencyGraph index = (graph, vertexToPkg, idToVertex)
  where
    ...
    resolve pid = pid
    edgesFrom pkg =
      ((), resolve (installedUnitId pkg), CD.flatDeps (depends pkg))
