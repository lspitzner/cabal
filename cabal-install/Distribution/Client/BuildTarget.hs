...
resolveUserBuildTargets pkgs utargets = do
  ...
  let
    ...
    (bproblems, btargets) = resolveBuildTargets primaryPkg otherPkgs utargets''
    -- default local dir target if there's no given target
    utargets''
      | not (null primaryPkg)
      , null utargets
      = [UserBuildTargetFileStatus1 "./" (FileStatusExistsDir pwd)]
      | otherwise
      = utargets'
  ...
  where
    selectPrimaryLocalPackage
      :: FilePath -> [PackageInfo] -> ([PackageInfo], [PackageInfo])
    selectPrimaryLocalPackage pwd pkgs' =
      let (primary, others) = partition isPrimary pkgs' in (primary, others)
      where
        isPrimary PackageInfo { pinfoDirectory = Just (dir, _) } | dir == pwd = True
        isPrimary _ = False
...
getUserTargetFileStatus t = case t of
  UserBuildTarget1 s1 ->
  ...
  where
    fileStatus f = do
      ...
      case splitPath f of
        _
          | fexists
          -> FileStatusExistsFile <$> canonicalizePath f
          | dexists
          -> FileStatusExistsDir <$> canonicalizePath f
...
--
parseUserBuildTargets
  :: [String] -> ([UserBuildTargetProblem], [UserBuildTarget])
...
parseUserBuildTarget targetstr =
  ...
  where
    ...
    readPToMaybe p str =
      listToMaybe [ r | (r, s) <- Parse.readP_to_S p str, all isSpace s ]
...
showBuildTarget ql =
  showUserBuildTarget . forgetFileStatus . head . renderBuildTarget ql
...
--
resolveBuildTargets
  :: [PackageInfo]     -- any primary pkg, e.g. cur dir
...
resolveBuildTargets ppinfo opinfo =
  partitionEithers . map (resolveBuildTarget ppinfo opinfo)
...
resolveBuildTarget ppinfo opinfo userTarget =
  case findMatch (matcher userTarget) of
    ...
    Ambiguous exactMatch targets ->
      case disambiguateBuildTargets matcher userTarget exactMatch targets of
  ...
  where
    ...
    classifyMatchErrors errs
      | not (null expected)
      = let (things, got:_) = unzip expected
        in  BuildTargetExpected userTarget' things got
      | not (null nosuch)
      ...
      where
        expected =
          [ (thing, got)
          | (_, MatchErrorExpected thing got) <- map (innerErr Nothing) errs
          ]
        nosuch =
          [ (inside, thing, got, alts)
          | (inside, MatchErrorNoSuch thing got alts) <- map
            (innerErr Nothing)
            errs
          ]

        innerErr _ (MatchErrorIn kind thing m) = innerErr (Just (kind, thing)) m
        innerErr c m                           = (c, m)
...
disambiguateBuildTargets matcher matchInput exactMatch matchResults =
  ...
  where
    ...
    matchResultsRenderings
      :: [(BuildTarget PackageInfo, [UserBuildTargetFileStatus])]
    matchResultsRenderings =
    ...
    memoisedMatches
      :: Map UserBuildTargetFileStatus (Match (BuildTarget PackageInfo))
    memoisedMatches =
        -- avoid recomputing the main one if it was an exact match
      ( if exactMatch
          then Map.insert matchInput (ExactMatch 0 matchResults)
          else id
        )
        $ Map.Lazy.fromList
    ...
    results
      = [ case findUnambiguous originalMatch matchRenderings of
          Just unambiguousRendering ->
            Right (forgetFileStatus unambiguousRendering, originalMatch)

          -- This case is an internal error, but we bubble it up and report it
          Nothing -> Left
            (originalMatch, [ (forgetFileStatus rendering, matches)
    ...
    findUnambiguous t (r:rs) = case memoisedMatches Map.! r of
      ExactMatch _ [t'] | fmap packageName t == fmap packageName t' -> Just r
      ExactMatch _ _ -> findUnambiguous t rs
      InexactMatch _ _ -> internalError "InexactMatch"
      NoMatch _ _ -> internalError "NoMatch"

internalError :: String -> a
internalError msg = error $ "BuildTargets: internal error: " ++ msg
...
renderBuildTarget
  :: QualLevel -> BuildTarget PackageInfo -> [UserBuildTargetFileStatus]
renderBuildTarget ql t = case t of
  BuildTargetPackage p     -> case ql of
    QL1 -> [t1 (dispP p)]
    QL2 -> [ t1' pf fs | (pf, fs) <- dispPF p ]
    QL3 -> []
    QL4 -> []

  BuildTargetComponent p c -> case ql of
    QL1 -> [t1 (dispC p c)]
    QL2 -> [t2 (dispP p) (dispC p c), t2 (dispK c) (dispC p c)]
    QL3 -> [t3 (dispP p) (dispK c) (dispC p c)]
    QL4 -> []

  BuildTargetModule p c m  -> case ql of
    QL1 -> [t1 (dispM m)]
    QL2 -> [t2 (dispP p) (dispM m), t2 (dispC p c) (dispM m)]
    QL3 ->
      [t3 (dispP p) (dispC p c) (dispM m), t3 (dispK c) (dispC p c) (dispM m)]
    QL4 -> [t4 (dispP p) (dispK c) (dispC p c) (dispM m)]

  BuildTargetFile   p c f  -> case ql of
    QL1 -> [t1 f]
    QL2 -> [t2 (dispP p) f, t2 (dispC p c) f]
    QL3 -> [t3 (dispP p) (dispC p c) f, t3 (dispK c) (dispC p c) f]
  ...
  where
    ...
    dispPF p =
      [ (addTrailingPathSeparator drel, FileStatusExistsDir dabs)
...
reportBuildTargetProblems problems = do
  ...
  case [ (t, m, ms) | MatchingInternalError t m ms <- problems ] of
    ...
    ((target, originalMatch, renderingsAndMatches):_) ->
      die
        $  "Internal error in build target matching. It should always be "
    ...
    targets -> die $ unlines
      ...
      , let
        groupByContainer =
          map
              ( \g@((inside, _, _, _):_) ->
                (inside, [ (thing, got, alts) | (_, thing, got, alts) <- g ])
              )
...
matchBuildTarget ppinfo opinfo =
  \utarget -> nubMatchesBy ((==) `on` (fmap packageName)) $ case utarget of
...
match1Pkg
  :: [PackageInfo] -> String -> FileStatus -> Match (BuildTarget PackageInfo)
...
match1Fil
  :: [PackageInfo] -> String -> FileStatus -> Match (BuildTarget PackageInfo)
match1Fil ps str1 fstatus1 = expecting "file" str1 $ do
...
match2KndCmp
  :: [ComponentInfo] -> String -> String -> Match (BuildTarget PackageInfo)
...
selectPackageInfo pkg loc = do
  (pkgdir, pkgfile) <- case loc of
    ...
    LocalUnpackedPackage dir -> do
      ...
      return
        (Just (dirabs, dirrel), if exists
          then Just (fileabs, filerel)
          else Nothing)
...
componentStringName
  :: Package pkg => pkg -> ComponentName -> ComponentStringName
...
componentHsFiles (CTest TestSuite { testInterface = TestSuiteExeV10 _ mainfile })
  = [mainfile]
componentHsFiles (CBench Benchmark { benchmarkInterface = BenchmarkExeV10 _ mainfile })
  = [mainfile]
componentHsFiles _ = []
...
matchComponentKind s
  | s `elem` ["lib", "library"]
  = increaseConfidence >> return LibKind
  | s `elem` ["exe", "executable"]
  = increaseConfidence >> return ExeKind
  | s `elem` ["tst", "test", "test-suite"]
  = increaseConfidence >> return TestKind
  | s `elem` ["bench", "benchmark"]
  = increaseConfidence >> return BenchKind
  | otherwise
  = matchErrorExpected "component kind" s
...
guardPackage str fstatus =
  guardPackageName str <|> guardPackageDir str fstatus <|> guardPackageFile
    str
    fstatus
...
validPackgageName s = all validPackgageNameChar s && not (null s)
  where
...
guardPackageFile _ (FileStatusExistsFile file) | takeExtension file == ".cabal" =
  increaseConfidence
...
matchPackageName ps = \str -> do
  ...
  orNoSuchThing "package" str (map (display . packageName) ps)
    $ increaseConfidenceFor
    $ matchInexactly caseFold (display . packageName) ps str


matchPackageDir :: [PackageInfo] -> String -> FileStatus -> Match PackageInfo
matchPackageDir ps = \str fstatus -> case fstatus of
...
guardComponentName s
  | all validComponentChar s && not (null s)
  = increaseConfidence
  | otherwise
...
matchComponentKindAndName
  :: [ComponentInfo] -> ComponentKind -> String -> Match ComponentInfo
matchComponentKindAndName cs ckind str =
  orNoSuchThing (showComponentKind ckind ++ " component") str (map render cs)
    $ increaseConfidenceFor
...
guardModuleName s = case simpleParse s :: Maybe ModuleName of
  Just _ -> increaseConfidence
  _
    | all validModuleChar s && not (null s)
    -> return ()
    | otherwise
    -> matchErrorExpected "module name" s
  where
...
matchPackageDirectoryPrefix
  :: [PackageInfo] -> FileStatus -> Match (FilePath, PackageInfo)
matchPackageDirectoryPrefix ps (FileStatusExistsFile filepath) =
  increaseConfidenceFor $ matchDirectoryPrefix pkgdirs filepath
  where
    pkgdirs =
      [ (dir, p) | p@PackageInfo { pinfoDirectory = Just (dir, _) } <- ps ]
matchPackageDirectoryPrefix _ _ = mzero


matchComponentFile
  :: [ComponentInfo] -> String -> Match (FilePath, ComponentInfo)
...
matchComponentOtherFile
  :: [ComponentInfo] -> String -> Match (FilePath, ComponentInfo)
matchComponentOtherFile cs = matchFile
  [ (file, c)
  | c    <- cs
  , file <- cinfoHsFiles c ++ cinfoCFiles c ++ cinfoJsFiles c
  ]
...
matchComponentModuleFile cs str = do
  matchFile [ (normalise (d </> toFilePath m), c)
            | c <- cs
            , d <- cinfoSrcDirs c
            , m <- cinfoModules c
            ]
...
matchFile fs = increaseConfidenceFor . matchInexactly caseFold fst fs

matchDirectoryPrefix :: [(FilePath, a)] -> FilePath -> Match (FilePath, a)
matchDirectoryPrefix dirs filepath =
  tryEach
    $ [ (file, x) | (dir, x) <- dirs, file <- maybeToList (stripDirectory dir) ]
  where
...
findMatch match = case nubMatchErrors match of
...
matchExactly key xs = \k -> case Map.lookup k m of
  Nothing -> mzero
...
--
matchInexactly
  :: (Ord k, Ord k') => (k -> k') -> (a -> k) -> [a] -> (k -> Match a)
matchInexactly cannonicalise key xs = \k -> case Map.lookup k m of
  Just ys -> exactMatches ys
