...
tryLoadSandboxConfig
  :: Verbosity -> GlobalFlags -> IO (FilePath, PackageEnvironment)
tryLoadSandboxConfig verbosity globalFlags = do
  path <- getSandboxConfigFilePath globalFlags
  tryLoadSandboxPackageEnvironmentFile verbosity
                                       path
...
tryGetIndexFilePath' globalFlags = do
  ...
  case paths of
    [] ->
      die
        $  "Distribution.Client.Sandbox.tryGetIndexFilePath: "
...
getSandboxPackageDB configFlags = do
  case configPackageDBs configFlags of
    ...
    [_] ->
      die
        $  "Unexpected contents of the 'package-db' field. "
        ++ sandboxConfigCorrupt
    _ -> die $ "Too many package DBs provided. " ++ sandboxConfigCorrupt
  where
...
withSandboxBinDirOnSearchPath sandboxDir = bracket_ addBinDir rmBinDir
  where
    ...
    addBinDir = do
      ...
      let
        newPath =
          maybe sandboxBin ((++) sandboxBin . (:) searchPathSeparator) mbOldPath
      setEnv "PATH" newPath
    ...
    rmBinDir   = do
      ...
      let newPath =
            intercalate [searchPathSeparator] (delete sandboxBin oldPath)
      setEnv "PATH" newPath
...
initPackageDBIfNeeded
  :: Verbosity -> ConfigFlags -> Compiler -> ProgramConfiguration -> IO ()
initPackageDBIfNeeded verbosity configFlags comp conf = do
  ...
  unless packageDBExists $ Register.initPackageDB verbosity comp conf dbPath
...
sandboxInit verbosity sandboxFlags globalFlags = do
  ...
  isCabalDevSandbox <- liftM2 (&&)
                              (doesDirectoryExist "cabal-dev")
  ...
  let sandboxDir' =
        fromFlagOrDefault defaultSandboxLocation (sandboxLocation sandboxFlags)
  ...
  if indexFileExists
  then
    notice verbosity $ "Using an existing sandbox located at " ++ sandboxDir
  else
    notice verbosity $ "Creating a new sandbox at " ++ sandboxDir
  Index.createEmpty verbosity indexFile

  -- Create the package DB for the default compiler.
  initPackageDBIfNeeded verbosity configFlags comp conf
  maybeAddCompilerTimestampRecord verbosity
                                  sandboxDir
                                  indexFile
                                  (compilerId comp)
                                  platform
...
sandboxDelete verbosity _sandboxFlags globalFlags = do
  ...
  case useSandbox of
    ...
    UseSandbox sandboxDir -> do
      ...
      -- location.
      let
        isNonDefaultConfigLocation =
          not
            $   equalFilePath pkgEnvFile
            $   curDir
      ...
      if isNonDefaultConfigLocation
        then
          warn verbosity
            $  "Sandbox config file is in non-default location: '"
            ++ pkgEnvFile
      ...
      let isNonDefaultSandboxLocation =
            not $ equalFilePath sandboxDir $ curDir </> defaultSandboxLocation
...
sandboxAddSource
  :: Verbosity -> [FilePath] -> SandboxFlags -> GlobalFlags -> IO ()
...
sandboxAddSourceSnapshot
  :: Verbosity -> [FilePath] -> FilePath -> PackageEnvironment -> IO ()
sandboxAddSourceSnapshot verbosity buildTreeRefs sandboxDir pkgEnv = do
  ...
  -- in consistent state.
  tmpDirs   <-
    forM (zip buildTreeRefs pkgs)
      $ \(buildTreeRef, pkg) -> inDir (Just buildTreeRef) $ do
          let targetDir    = snapshotDir </> (display . packageId $ pkg)
              targetTmpDir = targetDir ++ "-tmp"
          dirExists <- doesDirectoryExist targetTmpDir
          when dirExists $ removeDirectoryRecursive targetDir
...
sandboxDeleteSource verbosity buildTreeRefs _sandboxFlags globalFlags = do
  ...
  (results, convDict)  <- Index.removeBuildTreeRefs verbosity
                                                    indexFile
                                                    buildTreeRefs
  ...
  unless (null failedPaths) $ do
    ...
    die
      $  "The sources with the above errors were skipped. ("
  ...
  where
    ...
    handleErrors errs@(Index.ErrNonexistentSource{}:_) = warn
      verbosity
      (  "Source directory not found for paths: "
      ++ showE errs
      ++ "\n"
...
sandboxListSources :: Verbosity -> SandboxFlags -> GlobalFlags -> IO ()
sandboxListSources verbosity _sandboxFlags globalFlags = do
  ...
  when (not . null $ refs) $ do
    notice verbosity
      $  "Source dependencies registered "
...
updateInstallDirs
  :: Flag Bool -> (UseSandbox, SavedConfig) -> (UseSandbox, SavedConfig)
updateInstallDirs userInstallFlag (useSandbox, savedConfig) = case useSandbox of
  NoSandbox ->
...
loadConfigOrSandboxConfig verbosity globalFlags = do
  ...
  case pkgEnvType of
    ...
    AmbientPackageEnvironment -> do
      ...
      globalConstraintConfig <- loadUserConfig verbosity
                                               pkgEnvDir
  ...
  where
    ...
    dieIfSandboxRequired config = checkFlag flag
      where
        flag =
          (globalRequireSandbox . savedGlobalFlags $ config)
            `mappend` (globalRequireSandbox globalFlags)
        checkFlag (Flag True) =
          die
            $  "'require-sandbox' is set to True, but no sandbox is present. "
...
findSavedDistPref config flagDistPref = do
  let defDistPref = useDistPref defaultSetupScriptOptions
      flagDistPref' =
        configDistPref (savedConfigureFlags config) `mappend` flagDistPref
...
reinstallAddSourceDeps verbosity configFlags' configExFlags installFlags globalFlags sandboxDir
  = topHandler' $ do
    let sandboxDistPref = sandboxBuildDir sandboxDir
        configFlags     = configFlags' { configDistPref = Flag sandboxDistPref }
        haddockFlags    = mempty { haddockDistPref = Flag sandboxDistPref }
    (comp, platform, conf) <- configCompilerAux' configFlags
    retVal                 <- newIORef NoDepsReinstalled

    withSandboxPackageInfo verbosity
                           configFlags
                           globalFlags
                           comp
                           platform
          ...
          unless (null $ modifiedAddSourceDependencies sandboxPkgInfo) $ do
            ...
            withRepoContext verbosity globalFlags $ \repoContext -> do
              let
                args :: InstallArgs
                args =
                  ((configPackageDB' configFlags), repoContext, comp, platform, conf, UseSandbox
                    sandboxDir, Just sandboxPkgInfo, globalFlags, configFlags, configExFlags, installFlags, haddockFlags)
...
withSandboxPackageInfo
  ...
  -> IO ()
withSandboxPackageInfo verbosity configFlags globalFlags comp platform conf sandboxDir cont
  = do
  -- List all add-source deps.
    ...
    modifiedAddSourceDeps <- listModifiedDeps verbosity
                                              sandboxDir
                                              (compilerId comp)
                                              platform
                                              installedDepsMap
    ...
    let modifiedDeps =
          [ (modDepPath, fromJust $ M.lookup modDepPath depsMap)
          | modDepPath <- modifiedAddSourceDeps
          ]
        modifiedDepsMap = M.fromList modifiedDeps

    assert (all (`S.member`allAddSourceDepsSet) modifiedAddSourceDeps)
           (return ())
    if (null modifiedDeps)
      then info verbosity $ "Found no modified add-source deps."
      else
        notice verbosity
          $  "Some add-source dependencies have been modified. "
    ...
    cont $ SandboxPackageInfo (map toSourcePackage modifiedDeps)
                              (map toSourcePackage otherDeps)
                              installedPkgIndex
                              allAddSourceDepsSet
  where
    toSourcePackage (path, pkgDesc)
      = SourcePackage (packageId pkgDesc)
                      pkgDesc
...
maybeWithSandboxPackageInfo
  ...
  -> IO ()
maybeWithSandboxPackageInfo verbosity configFlags globalFlags comp platform conf useSandbox cont
  = case useSandbox of
...
maybeReinstallAddSourceDeps
  ...
  -> IO WereDepsReinstalled
maybeReinstallAddSourceDeps verbosity numJobsFlag configFlags' globalFlags' (useSandbox, config)
  = do
    case useSandbox of
      ...
      UseSandbox sandboxDir -> do
        ...
        let
          configFlags =
            savedConfigureFlags config `mappendSomeSavedFlags` configFlags'
          configExFlags =
            defaultConfigExFlags `mappend` savedConfigureExFlags config
          installFlags' = defaultInstallFlags `mappend` savedInstallFlags config
  ...
  where
    ...
    mappendSomeSavedFlags sandboxConfigFlags savedFlags = sandboxConfigFlags
...
configPackageDB' cfg = interpretPackageDbFlags userInstall
                                               (configPackageDBs cfg)
  where
    userInstall = fromFlagOrDefault True (configUserInstall cfg)

configCompilerAux'
  :: ConfigFlags -> IO (Compiler, Platform, ProgramConfiguration)
configCompilerAux' configFlags = configCompilerAuxEx configFlags
...
getPersistOrConfigCompiler configFlags = do
  ...
  case mlbi of
    ...
    Just lbi ->
      return
        (LocalBuildInfo.compiler lbi, LocalBuildInfo.hostPlatform lbi, LocalBuildInfo.withPrograms
          lbi)
