...
get verbosity repoCtxt globalFlags getFlags userTargets = do
  ...
  unless useFork $ mapM_ checkTarget userTargets
  ...
  pkgs          <- either (die . unlines . map show) return
    $ resolveWithoutDependencies (resolverParams sourcePkgDb pkgSpecifiers)

  unless (null prefix) $ createDirectoryIfMissing True prefix

  if useFork then fork pkgs else unpack pkgs
  where
...
forkPackage verbosity branchers prefix kind src = do
  ...
  case findBranchCmd branchers repos kind of
    ...
    Nothing             -> case repos of
      [] ->
        die ("Package " ++ pkgid ++ " does not have any source repositories.")
      _ ->
        die
          (  "Package "
          ++ pkgid
...
findBranchCmd
  ...
  -> (Maybe PD.RepoKind)
  -> Maybe BranchCmd
findBranchCmd branchers allRepos maybeKind = cmd
  where
...
branchGit = Brancher "git" $ \repo -> do
  ...
  let postClone dst = case PD.repoTag repo of
        Just t  -> do
          ...
          finally (rawSystem "git" (["checkout", t] ++ branchArgs))
