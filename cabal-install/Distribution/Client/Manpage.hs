...
commandDetailsLines pname (CommandSpec ui _ NormalCommand) =
  [".B " ++ pname ++ " " ++ (commandName ui), "", commandUsage ui pname, ""]
    ...
    ++ [".RE", ""]
  where
    optional field = case field ui of
      Just text -> [text pname, ""]
      Nothing   -> []
commandDetailsLines _ (CommandSpec _ _ HiddenCommand) = []

optionsLines :: CommandUI flags -> [String]
optionsLines command = concatMap
  optionLines
  (concatMap optionDescr (commandOptions command ParseArgs))
...
optionLines (ChoiceOpt options) = concatMap choiceLines options
  where
...
argOptionLines description optionChars optionStrings arg =
  [optionsLine optionChars optionStrings, optionArgLine arg]
    ++ optionDescriptionLines description
...
optionDescriptionLines description = [".RS", description, ".RE", ""]
...
fileLines (FileInfo path description) = [path, ".RS", description, ".RE", ""]
