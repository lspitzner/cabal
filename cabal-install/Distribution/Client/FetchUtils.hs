...
checkFetched loc = case loc of
  LocalUnpackedPackage dir  -> return (Just $ LocalUnpackedPackage dir)
  LocalTarballPackage  file -> return (Just $ LocalTarballPackage file)
  ...
  RepoTarballPackage repo pkgid Nothing -> fmap
...
fetchPackage verbosity repoCtxt loc = case loc of
  LocalUnpackedPackage dir             -> return (LocalUnpackedPackage dir)
  LocalTarballPackage  file            -> return (LocalTarballPackage file)
  RemoteTarballPackage uri (Just file) -> return (RemoteTarballPackage uri file)
  RepoTarballPackage repo pkgid (Just file) ->
...
fetchRepoTarball verbosity repoCtxt repo pkgid = do
  ...
  if fetched
    then do
      info verbosity $ display pkgid ++ " has already been downloaded."
      return (packageFile repo pkgid)
    else do
      setupMessage verbosity "Downloading" pkgid
      downloadRepoPackage
  where
...
asyncFetchPackages verbosity repoCtxt pkglocs body = do
  ...
  let fetchPackages :: IO ()
      fetchPackages = forM_ asyncDownloadVars $ \(pkgloc, var) -> do
        result <- try $ fetchPackage verbosity repoCtxt pkgloc
        putMVar var result

  withAsync fetchPackages $ \_ -> body (Map.fromList asyncDownloadVars)
...
packageURI repo pkgid = (remoteRepoURI repo)
  { uriPath = FilePath.Posix.joinPath
    [uriPath (remoteRepoURI repo), "package", display pkgid <.> "tar.gz"]
  }
