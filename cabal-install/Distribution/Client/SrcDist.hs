...
sdist flags exflags = do
  ...
  let withDir = if not needMakeArchive
        then (\f -> f tmpTargetDir)
  ...
  withDir $ \tmpDir -> do
    let
      outDir = if isOutDirectory then tmpDir else tmpDir </> tarBallName pkg
      flags'
        = ( if not needMakeArchive
            then flags
            else flags { sDistDirectory = Flag outDir }
          )
    unless isListSources $ createDirectoryIfMissingVerbose verbosity True outDir
    ...
    -- create an archive.
    when needMakeArchive $ createArchive verbosity pkg tmpDir distPref
...
createZipArchive verbosity pkg tmpDir targetPref = do
  ...
  hnd        <- runProcess (programPath zipProg)
                           ["-q", "-r", zipfileAbs, dir]
                           (Just tmpDir)
                           Nothing
                           Nothing
  ...
  unless (exitCode == ExitSuccess)
    $  die
    $  "Generating the zip file failed "
    ++ "(zip returned exit code "
    ++ show exitCode
    ++ ")"
...
allPackageSourceFiles
  :: Verbosity -> SetupScriptOptions -> FilePath -> IO [FilePath]
allPackageSourceFiles verbosity setupOpts0 packageDir = do
  ...
  withTempDirectory verbosity globalTmp "cabal-list-sources." $ \tempDir -> do
    let
      file      = tempDir </> "cabal-sdist-list-sources"
      flags     = defaultSDistFlags
        { sDistVerbosity   = Flag
          $ if verbosity == normal then lessVerbose verbosity else verbosity
      ...
      onFailedListSources e = do
        warn verbosity
          ...
          ++ "'."
        warn verbosity $ "Exception was: " ++ show e
