...
--   user.
extendFlags
  :: InstalledPackageIndex -> SourcePackageDb -> InitFlags -> IO InitFlags
...
f ?>> g = do
  ma <- f
  if isJust ma then return ma else g
...
getPackageName sourcePkgDb flags = do
  guess <-
    traverse guessPackageName (flagToMaybe $ packageDir flags)
      ?>>    Just
      `fmap` (getCurrentDirectory >>= guessPackageName)

  let guess'
        | isPkgRegistered guess
        = Nothing
        | otherwise
        = guess

  pkgName'    <-
    return (flagToMaybe $ packageName flags)
  ...
  where
    isPkgRegistered (Just pkg) =
      elemByPackageName (packageIndex sourcePkgDb) pkg
    isPkgRegistered Nothing = False

    promptOtherNameMsg =
      "This package name is already used by another "
...
getVersion flags = do
  ...
  v' <-
    return (flagToMaybe $ version flags)
...
getLicense flags = do
  lic <- return (flagToMaybe $ license flags) ?>> fmap
    (fmap (either UnknownLicense id))
    ( maybePrompt
      flags
...
getHomepage flags = do
  hp  <- queryHomepage
  hp' <-
    return (flagToMaybe $ homepage flags)
...
getCategory flags = do
  cat <- return (flagToMaybe $ category flags) ?>> fmap
    join
    (maybePrompt flags (promptListOptional "Project category" [Codec ..]))
...
getExtraSourceFiles flags = do
  extraSrcFiles <- return (extraSrc flags) ?>> Just `fmap` guessExtraSourceFiles
    flags
...
guessExtraSourceFiles flags = do
  ...
  where
    ...
    likeFileNameBase candidates =
      (`elem`candidates) . map toUpper . takeBaseName
...
getLibOrExec flags = do
  isLib    <-
    return (flagToMaybe $ packageType flags)
      ?>> maybePrompt flags
                      ( either (const Library) id `fmap` promptList
      ...
      ?>> return (Just Library)
  mainFile <-
    if isLib /= Just Executable then return Nothing else getMainFile flags

  return
    $ flags { packageType = maybeToFlag isLib, mainIs = maybeToFlag mainFile }
...
getMainFile flags = return (flagToMaybe $ mainIs flags) ?>> do
  ...
  maybePrompt flags
              ( either id (either id id) `fmap` promptList
...
getLanguage flags = do
  lang <-
    return (flagToMaybe $ language flags)
      ?>> maybePrompt flags
                      ( either UnknownLanguage id `fmap` promptList
...
getGenComments flags = do
  genComments <-
    return (not <$> flagToMaybe (noComments flags))
...
guessSourceDir flags = do
  ...
  return $ if srcIsDir then Just "src" else Nothing
...
isSourceFile (Just srcDirs) sf =
  any (equalFilePath (relativeSourcePath sf)) srcDirs

-- | Get the list of exposed modules and extra tools needed to build them.
getModulesBuildToolsAndDeps
  :: InstalledPackageIndex -> InitFlags -> IO InitFlags
getModulesBuildToolsAndDeps pkgIx flags = do
  ...
  Just mods <-
    return (exposedModules flags)
      ?>> (return . Just . map moduleName $ sourceFiles)

  tools     <-
    return (buildTools flags)
      ?>> (return . Just . neededBuildPrograms $ sourceFiles)

  deps      <- return (dependencies flags) ?>> Just <$> importsToDeps
    flags
    ( fromString "Prelude"
    :  -- to ensure we get base as a dep
      (nub   -- only need to consider each imported package once
           . filter (`notElem`mods)  -- don't consider modules from
                                                        -- this package itself
                                    . concatMap imports $ sourceFiles)
    )
    pkgIx

  exts      <-
    return (otherExts flags)
...
importsToDeps
  :: InitFlags -> [ModuleName] -> InstalledPackageIndex -> IO [P.Dependency]
...
chooseDep flags (m, Just ps) = case pkgGroups of
  ...
  where
    ...
    toDep [pid] =
      return $ P.Dependency (P.pkgName pid) (pvpize . P.pkgVersion $ pid)

    -- Otherwise, choose the latest version and issue a warning.
    toDep pids = do
      message flags
              (  "\nWarning: multiple versions of "
              ++ display (P.pkgName . head $ pids)
              ++ " provide "
              ++ display m
              ++ ", choosing the latest."
              )
...
pvpize v =
  orLaterVersion v' `intersectVersionRanges` earlierVersion (incVersion 1 v')
  where
    v' = (v { versionBranch = take 2 (versionBranch v) })
...
maybePrompt flags p = case nonInteractive flags of
...
promptYesNo = promptDefault' recogniseYesNo showYesNo
  where
    recogniseYesNo s
      | s == "y" || s == "Y"
      = Just True
...
promptDefault' parser pretty pr def = do
  ...
  case (inp, def) of
    ...
    _            -> case parser inp of
      Just t  -> return t
      Nothing -> do
        putStrLn $ "Couldn't parse " ++ inp ++ ", please try again!"
...
promptListOptional'
  ...
  -> IO (Maybe (Either String t))
promptListOptional' pr choices displayItem = fmap rearrange $ promptList
  pr
  (Nothing : map Just choices)
  (Just Nothing)
  (maybe "(none)" displayItem)
  True
  where
...
promptList pr choices def displayItem other = do
  putStrLn $ pr ++ ":"
  let
    options1 = map (\c -> (Just c == def, displayItem c)) choices
    options2 = zip ([1 ..] :: [Int])
...
promptList'
  :: (t -> String) -> Int -> [t] -> Maybe t -> Bool -> IO (Either String t)
promptList' displayItem numChoices choices def other = do
  ...
  case (inp, def) of
    ...
    _            -> case readMaybe inp of
      ...
      Just n  -> getChoice n
  where
    invalidChoice inp = do
      putStrLn $ inp ++ " is not a valid choice."
      promptList' displayItem numChoices choices def other
    getChoice n
      | n < 1 || n > numChoices
      = invalidChoice (show n)
      | n < numChoices || (n == numChoices && not other)
      = return . Right $ choices !! (n - 1)
      | otherwise
      = Left `fmap` promptStr "Please specify" Nothing
...
writeLicense flags = do
  ...
  let
    licenseFile = case license flags of
      Flag BSD2 -> Just $ bsd2 authors year

      Flag BSD3 -> Just $ bsd3 authors year

      Flag (GPL (Just (Version { versionBranch = [2] }))) -> Just gplv2

      Flag (GPL (Just (Version { versionBranch = [3] }))) -> Just gplv3

      Flag (LGPL (Just (Version { versionBranch = [2, 1] }))) -> Just lgpl21

      Flag (LGPL (Just (Version { versionBranch = [3] }))) -> Just lgpl3

      Flag (AGPL (Just (Version { versionBranch = [3] }))) -> Just agplv3

      Flag (Apache (Just (Version { versionBranch = [2, 0] }))) -> Just apache20

      Flag MIT -> Just $ mit authors year

      Flag (MPL (Version { versionBranch = [2, 0] })) -> Just mpl20

      Flag ISC -> Just $ isc authors year
...
writeSetupFile flags = do
  ...
  where
    setupFile = unlines ["import Distribution.Simple", "main = defaultMain"]

writeChangeLog :: InitFlags -> IO ()
writeChangeLog flags =
  when (any (==defaultChangeLog) $ maybe [] id (extraSrc flags)) $ do
...
createMainHs flags@InitFlags { sourceDirs = Just (srcPath:_), packageType = Flag Executable, mainIs = Flag mainFile }
  = writeMainHs flags (srcPath </> mainFile)
createMainHs flags@InitFlags { sourceDirs = _, packageType = Flag Executable, mainIs = Flag mainFile }
...
moveExistingFile flags fileName = unless (overwrite flags == Flag True) $ do
...
generateCabalFile fileName c =
  (++"\n")
    ...
    $$ vcat
         [ field "name" (packageName c) (Just "The name of the package.") True
         , field
           "version"
           (version c)
           (  Just
           $ "The package version.  See the Haskell package versioning policy (PVP) for standards guiding when and how versions should be incremented.\nhttps://wiki.haskell.org/Package_versioning_policy\n"
         ...
         , case (license c) of
           ...
           _ -> fieldS "license-file"
                       (Flag "LICENSE")
                       (Just "The file containing the license text.")
                       True
         , fieldS "author" (author c) (Just "The package author(s).") True
         , fieldS
           "maintainer"
           (email c)
           ( Just
         ...
         , case (license c) of
           Flag PublicDomain -> empty
           _ -> fieldS "copyright" NoFlag (Just "A copyright notice.") True
         , fieldS "category" (either id display `fmap` category c) Nothing True
         , fieldS "build-type" (Flag "Simple") Nothing True
         , fieldS
           "extra-source-files"
           (listFieldS (extraSrc c))
           ( Just
         ...
         , field
           "cabal-version"
           (Flag $ orLaterVersion (Version [1, 10] []))
           ( Just
         ...
         , case packageType c of
               ...
               $$  nest
                     ...
                     ( vcat
                       [ fieldS
                         "main-is"
                         (mainIs c)
                         (Just ".hs or .lhs file containing the Main module.")
                         True
           ...
           Flag Library -> text "\nlibrary" $$ nest
             2
             ( vcat
               [ fieldS "exposed-modules"
                        (listField (exposedModules c))
  ...
  where
    ...
    fieldS s f com _ =
      case (isJust com, noComments c, minimal c) of
          (_    , _        , Flag True) -> id
          (_    , Flag True, _        ) -> id
    ...
    showComment (Just t) =
      vcat
        . map (text . ("-- "++))
        . lines
        . renderStyle style { lineLength = 76, ribbonsPerLine = 1.05 }
        . vcat
    ...
    breakLine [] = []
    breakLine cs = case break (==' ') cs of
      (w, cs') -> w : breakLine' cs'
    breakLine' [] = []
    breakLine' cs = case span (==' ') cs of
      (w, cs') -> w : breakLine cs'
...
generateWarnings flags = do
  message flags ""
  when
    (synopsis flags `elem` [NoFlag, Flag ""])
    ( message
      flags
