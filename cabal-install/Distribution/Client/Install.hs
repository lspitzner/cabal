...
install
  ...
  -> [UserTarget]
  -> IO ()
install verbosity packageDBs repos comp platform conf useSandbox mSandboxPkgInfo globalFlags configFlags configExFlags installFlags haddockFlags userTargets0
  = do
  ...
  where
    args :: InstallArgs
    args =
      (packageDBs, repos, comp, platform, conf, useSandbox, mSandboxPkgInfo, globalFlags, configFlags, configExFlags, installFlags, haddockFlags)

    die' message =
      die
        (  message
        ++ if isUseSandbox useSandbox then installFailedInSandbox else []
        )
...
makeInstallContext
  :: Verbosity -> InstallArgs -> Maybe [UserTarget] -> IO InstallContext
makeInstallContext verbosity (packageDBs, repoCtxt, comp, _, conf, _, _, globalFlags, _, configExFlags, _, _) mUserTargets
  = do
    ...
    return
      (installedPkgIndex, sourcePkgDb, pkgConfigDb, userTargets, pkgSpecifiers, transport)
...
makeInstallPlan verbosity (_, _, comp, platform, _, _, mSandboxPkgInfo, _, configFlags, configExFlags, installFlags, _) (installedPkgIndex, sourcePkgDb, pkgConfigDb, _, pkgSpecifiers, _)
  = do
...
processInstallPlan
  :: Verbosity -> InstallArgs -> InstallContext -> SolverInstallPlan -> IO ()
processInstallPlan verbosity args@(_, _, _, _, _, _, _, _, _, _, installFlags, _) (installedPkgIndex, sourcePkgDb, _, userTargets, pkgSpecifiers, _) installPlan0
  = do
...
planPackages comp platform mSandboxPkgInfo solver configFlags configExFlags installFlags installedPkgIndex sourcePkgDb pkgConfigDb pkgSpecifiers
    ...
    resolverParams =
      ...
      setMaxBackjumps (if maxBackjumps < 0 then Nothing else Just maxBackjumps)
        ...
        . setPreferenceDefault
            (if upgradeDeps then PreferAllLatest else PreferLatestForSelected)
        ...
        . addConstraints
          ...
          -- is silly. We should check if the flags are appropriate
            [ let
              ...
              in
                LabeledPackageConstraint pc ConstraintSourceConfigFlagOrTarget
        ...
        . addConstraints
            [ let
                pc = PackageConstraintStanzas (pkgSpecifierTarget pkgSpecifier)
                                              stanzas
              in
                LabeledPackageConstraint pc ConstraintSourceConfigFlagOrTarget
            | pkgSpecifier <- pkgSpecifiers
...
checkPrintPlan
  ...
  -> [PackageSpecifier UnresolvedSourcePackage]
  -> IO ()
checkPrintPlan verbosity installed installPlan sourcePkgDb installFlags pkgSpecifiers
  = do

  -- User targets that are already installed.
    let preExistingTargets =
          [ p
          | let tgts = map pkgSpecifierTarget pkgSpecifiers
    ...
    let reinstalledPkgs =
          [ ipkg | (_pkg, status) <- lPlan, ipkg <- extractReinstalls status ]
    ...
    when containsReinstalls $ do
      if breaksPkgs
        then do
          (if dryRun || overrideReinstall then warn verbosity else die)
            $  unlines
...
packageStatus :: InstalledPackageIndex -> ReadyPackage -> PackageStatus
packageStatus installedPkgIndex cpkg =
  case PackageIndex.lookupPackageName installedPkgIndex (packageName cpkg) of
    [] -> NewPackage
    ps ->
      case
          filter ((==packageId cpkg) . Installed.sourcePackageId)
                 (concatMap snd ps)
        of
          [] -> NewVersion (map fst ps)
          pkgs@(pkg:_) ->
            Reinstall (map Installed.installedUnitId pkgs) (changes pkg cpkg)
  where
    ...
    changes pkg pkg' = filter changed $ mergeBy
...
printPlan dryRun verbosity plan sourcePkgDb = case plan of
  ...
  where
    ...
    showPkg (pkg, _) = display (packageId pkg) ++ showLatest (pkg)
      ...
      display (packageId pkg')
        ...
        ++ case pr of
             ...
             Reinstall _ cs -> " (reinstall)" ++ case cs of
               [] -> ""
               diff ->
                 " (changes: " ++ intercalate ", " (map change diff) ++ ")"

    showLatest :: Package srcpkg => srcpkg -> String
    showLatest pkg = case mLatestVersion of
      Just latestVersion -> if packageVersion pkg < latestVersion
    ...
    nonDefaultFlags cpkg =
      let
        defaultAssignment =
          toFlagAssignment
            ( genPackageFlags
              (SourcePackage.packageDescription $ confPkgSource cpkg)
            )
      in
        confPkgFlags cpkg \\ defaultAssignment
    ...
    change (InBoth pkgid pkgid') =
      display pkgid ++ " -> " ++ display (packageVersion pkgid')
...
reportPlanningFailure verbosity (_, _, comp, platform, _, _, _, _, configFlags, _, installFlags, _) (_, sourcePkgDb, _, _, pkgSpecifiers, _) message
  = do
    ...
    when reportFailure $ do
      ...
      let
        ...
        buildReports = BuildReports.fromPlanningFailure
          platform
          (compilerId comp)
          pkgids
      ...
      case logFile of
        ...
        Just template -> forM_ pkgids $ \pkgid ->
          let
            env  = initialPathTemplateEnv pkgid
                                          dummyIpid
                                          (compilerInfo comp)
                                          platform
...
theSpecifiedPackage pkgSpec = case pkgSpec of
  NamedPackage name [PackageConstraintVersion name' version] | name == name' ->
  ...
  where
    ...
    trivialRange = foldVersionRange Nothing
                                    Just     -- "== v"
                                    (\_ -> Nothing)
                                    (\_ -> Nothing)
                                    (\_ _ -> Nothing)
...
postInstallActions
  ...
  -> BuildResults
  -> IO ()
postInstallActions verbosity (packageDBs, _, comp, platform, conf, useSandbox, mSandboxPkgInfo, globalFlags, configFlags, _, installFlags, _) targets installPlan buildResults
  = do

    unless oneShot $ World.insert verbosity
                                  worldFile
    ...
    let buildReports = BuildReports.fromInstallPlan platform
                                                    (compilerId comp)
                                                    installPlan
                                                    buildResults
    BuildReports.storeLocal (compilerInfo comp)
    ...
    updateSandboxTimestampsFile useSandbox
                                mSandboxPkgInfo
                                comp
                                platform
                                installPlan
...
storeDetailedBuildReports
  :: Verbosity -> FilePath -> [(BuildReports.BuildReport, Maybe Repo)] -> IO ()
storeDetailedBuildReports verbosity logsDir reports = sequence_
  ...
  where
    ...
    handleMissingLogFile = Exception.handleJust missingFile $ \ioe ->
      warn verbosity $ "Missing log file for build report: " ++ fromMaybe
        ""
        (ioeGetFileName ioe)

    missingFile ioe | isDoesNotExistError ioe = Just ioe
...
regenerateHaddockIndex
  ...
  -> BuildResults
  -> IO ()
regenerateHaddockIndex verbosity packageDBs comp platform conf useSandbox configFlags installFlags buildResults
  ...
  = do
    ...
    notice verbosity $ "Updating documentation index " ++ indexFile
  ...
  where
    ...
    shouldRegenerateHaddockIndex =
      (isUseSandbox useSandbox || normalUserInstall)
    ...
    substHaddockIndexFileName defaultDirs = fromPathTemplate
      ...
      where
        ...
        absoluteDirs =
          InstallDirs.substituteInstallDirTemplates env0 templateDirs
...
symlinkBinaries
  ...
  -> BuildResults
  -> IO ()
symlinkBinaries verbosity platform comp configFlags installFlags plan buildResults
  = do
    failed <- InstallSymlink.symlinkBinaries platform
                                             comp
                                             configFlags
                                             installFlags
                                             plan
...
printBuildFailures buildResults
  = case
      [ (pkgid, failure) | (pkgid, Left failure) <- Map.toList buildResults ]
    of
      [] -> return ()
      failed ->
        die
  ...
  where
    printFailureReason reason = case reason of
      DependentFailed pkgid ->
        " depends on " ++ display pkgid ++ " which failed to install."
      DownloadFailed e ->
        " failed while downloading the package." ++ showException e
      UnpackFailed e ->
        " failed while unpacking the package." ++ showException e
      ConfigureFailed e ->
        " failed during the configure step." ++ showException e
      BuildFailed e -> " failed during the building phase." ++ showException e
      TestsFailed e -> " failed during the tests phase." ++ showException e
      InstallFailed e ->
        " failed during the final install step." ++ showException e
    ...
    onExitFailure (ExitFailure n) | n == 9 || n == -9 =
...
updateSandboxTimestampsFile
  ...
  -> BuildResults
  -> IO ()
updateSandboxTimestampsFile (UseSandbox sandboxDir) (Just (SandboxPackageInfo _ _ _ allAddSourceDeps)) comp platform installPlan buildResults
  = withUpdateTimestamps sandboxDir (compilerId comp) platform $ \_ -> do
    let
      ...
      allPaths =
        [ pth | LocalUnpackedPackage pth <- map packageSource allSrcPkgs ]
...
performInstallations
  ...
  -> InstallPlan
  -> IO BuildResults
performInstallations verbosity (packageDBs, repoCtxt, comp, platform, conf, useSandbox, _, globalFlags, configFlags, configExFlags, installFlags, haddockFlags) installedPkgIndex installPlan
  = do
  ...
  -- With 'install -j' it can be a bit hard to tell whether a sandbox is used.
    ...
    executeInstallPlan verbosity jobControl keepGoing useLogFile installPlan
      $ \rpkg ->
          installReadyPackage platform cinfo configFlags rpkg
            $ \configFlags' src pkg pkgoverride ->
                fetchSourcePackage verbosity repoCtxt fetchLimit src
                  $ \src' ->
                      installLocalPackage verbosity
                                          (packageId pkg)
                                          src'
                                          distPref
                        $ \mpath -> installUnpackedPackage
                            verbosity
                            installLock
                            numJobs
  ...
  where
    ...
    setupScriptOptions index lock rpkg = configureSetupScript
      packageDBs
      comp
      platform
      conf
    ...
    useLogFile = fmap ((\f -> (f, loggingVerbosity)) . substLogFileName)
      ...
      where
        ...
        loggingVerbosity
          | overrideVerbosity
          = max Verbosity.verbose verbosity
          | otherwise
          = verbosity
    ...
    substLogFileName template pkg ipid =
      fromPathTemplate . substPathTemplate env $ template
      where
        env
          = initialPathTemplateEnv (packageId pkg)
                                   ipid
                                   (compilerInfo comp)
                                   platform
...
executeInstallPlan verbosity jobCtl keepGoing useLogFile plan0 installPkg =
  InstallPlan.execute jobCtl keepGoing depsFailure plan0 $ \pkg -> do
  ...
  where
    ...
    printBuildResult pkgid ipid buildResult = case buildResult of
      ...
      (Left  _) -> do
        ...
        when (verbosity >= normal) $ case useLogFile of
          Nothing                 -> return ()
...
installReadyPackage
  ...
  -> a
installReadyPackage platform cinfo configFlags (ReadyPackage (ConfiguredPackage ipid (SourcePackage _ gpkg source pkgoverride) flags stanzas deps)) installPkg
  = installPkg
    configFlags
...
fetchSourcePackage verbosity repoCtxt fetchLimit src installPkg = do
  ...
  case fetched of
    ...
    Nothing   -> onFailure DownloadFailed $ do
      loc <- withJobLimit fetchLimit $ fetchPackage verbosity repoCtxt src
      installPkg loc
...
installLocalTarballPackage
  ...
  -> (Maybe FilePath -> IO BuildResult)
  -> IO BuildResult
installLocalTarballPackage verbosity pkgid tarballPath distPref installPkg = do
  tmp <- getTemporaryDirectory
  withTempDirectory verbosity tmp "cabal-tmp"
    $ \tmpDirPath -> onFailure UnpackFailed $ do
        let
          relUnpackedPath = display pkgid
          absUnpackedPath = tmpDirPath </> relUnpackedPath
          descFilePath =
            absUnpackedPath </> display (packageName pkgid) <.> "cabal"
        info verbosity
          $  "Extracting "
          ++ tarballPath
          ++ " to "
        ...
        when (not exists) $ die $ "Package .cabal file not found: " ++ show
          descFilePath
  ...
  where
    ...
    maybeRenameDistDir absUnpackedPath = do
      ...
      when (distDirExists && (not $ distDirPath `equalFilePath` distDirPathNew))
        $ do
...
installUnpackedPackage
  ...
  -> IO BuildResult
installUnpackedPackage verbosity installLock numJobs scriptOptions configFlags installFlags haddockFlags comp conf platform pkg rpkg pkgoverride workingDir useLogFile
  = do
  -- Override the .cabal file if necessary
    case pkgoverride of
      ...
      Just pkgtxt -> do
        let
          descFilePath =
            fromMaybe "." workingDir </> display (packageName pkgid) <.> "cabal"
        info verbosity
          $   "Updating "
      ...
      -- Tests phase
          ...
          -- Install phase
            onFailure InstallFailed $ criticalSection installLock $ do
              -- Actual installation
              withWin32SelfUpgrade verbosity ipid configFlags cinfo platform pkg
                $ do
              ...
              forM_ ipkgs'
                $ \ipkg' -> registerPackage verbosity
                                            comp
                                            conf
                                            NoMultiInstance
                                            packageDBs
  ...
  where
    ...
    buildFlags _ = emptyBuildFlags { buildDistPref  = configDistPref configFlags
    ...
    testFlags _ =
      Cabal.emptyTestFlags { Cabal.testDistPref = configDistPref configFlags }
    ...
    genPkgConfs :: Maybe FilePath -> IO [Installed.InstalledPackageInfo]
    genPkgConfs mLogPath = if shouldRegister
      then do
        tmp <- getTemporaryDirectory
        withTempDirectory verbosity tmp (tempTemplate "pkgConf") $ \dir -> do
          let
            pkgConfDest = dir </> "pkgConf"
    ...
    readPkgConf :: FilePath -> FilePath -> IO Installed.InstalledPackageInfo
    ...
    maybeLogPath = case useLogFile of
    ...
    setup cmd flags mLogPath = Exception.bracket
      (traverse (\path -> openFile path AppendMode) mLogPath)
      (traverse_ hClose)
      ( \logFileHandle -> setupWrapper verbosity
                                       scriptOptions
    ...
    exeInstallPaths defaultDirs =
      ...
      where
        ...
        templateDirs     = InstallDirs.combineInstallDirs
          ...
          defaultDirs
          (configInstallDirs configFlags)
        absoluteDirs = InstallDirs.absoluteInstallDirs pkgid
                                                       ipid
                                                       cinfo
