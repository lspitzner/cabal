...
fetch verbosity packageDBs repoCtxt comp platform conf globalFlags fetchFlags userTargets
  = do
...
planPackages verbosity comp platform fetchFlags installedPkgIndex sourcePkgDb pkgConfigDb pkgSpecifiers
  | includeDependencies
  = do
    solver <-
    ...
    return
      ...
      | (SolverInstallPlan.Configured cpkg) <- SolverInstallPlan.toList
        installPlan
      ]
  | otherwise
