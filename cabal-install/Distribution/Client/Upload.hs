...
upload verbosity repoCtxt mUsername mPassword isCandidate paths = do
  ...
  forM_ paths $ \path -> do
    ...
    case fmap takeFileName (stripExtensions ["tar", "gz"] path) of
      Just pkgid -> handlePackage transport
                                  verbosity
                                  uploadURI
                                  (packageURI pkgid)
                                  auth
...
handlePackage
  ...
  -> FilePath
  -> IO ()
handlePackage transport verbosity uri packageUri auth isCandidate path = do
