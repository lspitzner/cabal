...
getPkgList verbosity packageDBs repoCtxt comp conf listFlags pats = do
  ...
  let
    sourcePkgIndex = packageIndex sourcePkgDb
    prefs name =
      fromMaybe anyVersion (Map.lookup name (packagePreferences sourcePkgDb))
    ...
    pkgsInfo
      ...
      = mergePackages (InstalledPackageIndex.allPackages installedPkgIndex)
    ...
    pkgsInfoMatching =
      let
        matchingInstalled = matchingPackages
          InstalledPackageIndex.searchByNameSubstring
          installedPkgIndex
        matchingSource    = matchingPackages
          (\idx n -> concatMap snd (PackageIndex.searchByNameSubstring idx n))
          sourcePkgIndex
      in
        mergePackages matchingInstalled matchingSource

    matches :: [PackageDisplayInfo]
    matches =
      [ mergePackageInfo pref installedPkgs sourcePkgs selectedPkg False
  ...
  where
    ...
    matchingPackages search index =
      [ pkg | pat <- pats, pkg <- search index pat ]
  ...
  if simpleOutput
    then putStr $ unlines
      ...
      , version <- if onlyInstalled
        ...
        else nub . sort $ installedVersions pkg ++ sourceVersions pkg
      ]
...
info verbosity packageDBs repoCtxt comp conf globalFlags _listFlags userTargets
  = do
    ...
    pkgsinfo      <- sequence
      [ do
        pkginfo <- either die return
          $ gatherPkgInfo prefs installedPkgIndex sourcePkgIndex pkgSpecifier
  ...
  where
    ...
    gatherPkgInfo prefs installedPkgIndex sourcePkgIndex (NamedPackage name constraints)
      ...
      where
        ...
        selectedSourcePkgs = PackageIndex.lookupDependency
          sourcePkgIndex
          (Dependency name verConstraint)
    ...
    gatherPkgInfo prefs installedPkgIndex sourcePkgIndex (SpecificSourcePackage pkg)
      = Right $ mergePackageInfo pref installedPkgs sourcePkgs selectedPkg True
      where
...
sourcePkgsInfo prefs name installedPkgIndex sourcePkgIndex =
  ...
  where
    pref          = prefs name
    installedPkgs = concatMap
      snd
      (InstalledPackageIndex.lookupPackageName installedPkgIndex name)
...
showPackageSummaryInfo pkginfo =
  renderStyle (style { lineLength = 80, ribbonsPerLine = 1 })
    $   char '*'
    <+> disp (pkgName pkginfo)
    $+$ ( nest 4 $ vcat
          [ maybeShow (synopsis pkginfo) "Synopsis:" reflowParagraphs
          , text "Default available version:"
            <+> case selectedSourcePkg pkginfo of
                  Nothing ->
                    text "[ Not available from any configured repository ]"
                  Just pkg -> disp (packageVersion pkg)
          , text "Installed versions:" <+> case installedVersions pkginfo of
            []
              | hasLib pkginfo
              -> text "[ Not installed ]"
              | otherwise
              -> text "[ Unknown ]"
            versions -> dispTopVersions 4 (preferredVersions pkginfo) versions
...
showPackageDetailedInfo pkginfo =
  ...
  where
    ...
    pkgkind
      | hasLib pkginfo && hasExes
      = text "programs and library"
      | hasLib pkginfo && hasExe pkginfo
      = text "program and library"
      | hasLib pkginfo
      = text "library"
      | hasExes
      = text "programs"
...
mergePackageInfo versionPref installedPkgs sourcePkgs selectedPkg showVer =
  assert (length installedPkgs + length sourcePkgs > 0) $ PackageDisplayInfo
    { pkgName           = combine packageName source packageName installed
    , selectedVersion   = if showVer
    ...
    , license = combine Source.license source Installed.license installed
    , maintainer        = combine Source.maintainer
                                  source
                                  Installed.maintainer
                                  installed
    , author = combine Source.author source Installed.author installed
    , homepage = combine Source.homepage source Installed.homepage installed
    , bugReports        = maybe "" Source.bugReports source
    , sourceRepo        = fromMaybe ""
      . join
      . fmap
    ...
    , description       = combine Source.description
                                  source
                                  Installed.description
                                  installed
    , category = combine Source.category source Installed.category installed
    , flags             = maybe [] Source.genPackageFlags sourceGeneric
    , hasLib            = isJust installed
      || fromMaybe False (fmap (isJust . Source.condLibrary) sourceGeneric)
    , hasExe            = fromMaybe False
                                    ( fmap (not . null . Source.condExecutables)
                                           sourceGeneric
                                    )
    ...
    , dependencies      = combine
      (map (SourceDependency . simplifyDependency) . Source.buildDepends)
      source
      (map InstalledDependency . Installed.depends)
      installed
    , haddockHtml       = fromMaybe ""
      . join
  ...
  where
    ...
    getListOfExposedModules lib = Source.exposedModules lib
      ++ map Source.moduleReexportName (Source.reexportedModules lib)

    sourceSelected
      | isJust selectedPkg
      = selectedPkg
      | otherwise
      = latestWithPref versionPref sourcePkgs
...
updateFileSystemPackageDetails pkginfo = do
  fetched   <-
    maybe (return False) (isFetched . packageSource) (selectedSourcePkg pkginfo)
  docsExist <- doesDirectoryExist (haddockHtml pkginfo)
  return pkginfo { haveTarball = fetched
...
latestWithPref pref pkgs = Just (maximumBy (comparing prefThenVersion) pkgs)
  where
    prefThenVersion pkg =
      let ver = packageVersion pkg in (withinRange ver pref, ver)
...
dispTopVersions n pref vs =
  ...
  where
    ...
    trailingMessage
      ...
      <+> if extra == 1 then Disp.text "other" else Disp.text "others"
...
interestingVersions pref =
  ...
  where
    ...
    mkTree xs = unfoldTree step (False, [], xs)
      where
        step (node, ns, vs) =
          ((reverse ns, node), [ (any null vs', n : ns, filter (not . null) vs')
                               | (n, vs') <- groups vs
                               ])
        groups =
          map (\g -> (head (head g), map tail g)) . groupBy (equating head)
