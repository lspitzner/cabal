...
-- directory?
classifyPackageEnvironment
  :: FilePath -> Flag FilePath -> Flag Bool -> IO PackageEnvironmentType
classifyPackageEnvironment pkgEnvDir sandboxConfigFileFlag ignoreSandboxFlag =
  do
    isSandbox <- liftM2 (||)
                        (return forceSandboxConfig)
  ...
  where
    ...
    classify True _ | not ignoreSandbox = SandboxPackageEnvironment
...
commonPackageEnvironmentConfig sandboxDir = mempty
...
commonPackageEnvironment sandboxDir =
  mempty { pkgEnvSavedConfig = commonPackageEnvironmentConfig sandboxDir }
...
sandboxInstallDirs sandboxDir =
  mempty { prefix = toFlag (toPathTemplate sandboxDir) }
...
basePackageEnvironment = mempty
  { pkgEnvSavedConfig = mempty
    { savedConfigureFlags = mempty { configHcFlavor  = toFlag defaultCompiler
...
initialPackageEnvironment
  :: FilePath -> Compiler -> Platform -> IO PackageEnvironment
initialPackageEnvironment sandboxDir compiler platform = do
  ...
  let initialConfig = commonPackageEnvironmentConfig sandboxDir
      installDirs   = combineInstallDirs (\d f -> Flag $ fromFlagOrDefault d f)
                                         defInstallDirs
                                         (savedUserInstallDirs initialConfig)
  return $ mempty
...
setPackageDB sandboxDir compiler platform configFlags = configFlags
...
overrideSandboxSettings pkgEnv0 pkgEnv = pkgEnv
...
inheritedPackageEnvironment
  :: Verbosity -> PackageEnvironment -> IO PackageEnvironment
  ...
  case (minp, globalConfigLocation) of
    ...
    (_, Just globalLoc) ->
      maybe
        ...
        =<< readPackageEnvironmentFile (ConstraintSourceUserConfig globalLoc)
                                       mempty
                                       globalLoc
    _ -> return mempty
  where
...
handleParseResult verbosity path minp = case minp of
  Nothing -> die $ "The package environment file '" ++ path ++ "' doesn't exist"
...
tryLoadSandboxPackageEnvironmentFile verbosity pkgEnvFile configFileFlag = do
  ...
  minp   <- readPackageEnvironmentFile
    (ConstraintSourceSandboxConfig pkgEnvFile)
    mempty
    pkgEnvFile
  ...
  let sandboxDir =
        fromFlagOrDefault defaultSandboxLocation
  ...
  unless dirExists $ die ("No sandbox exists at " ++ sandboxDir)
  ...
  return
    (sandboxDir, updateInstallDirs
      $                         (         base
                                `mappend` (toPkgEnv cabalConfig)
                                `mappend` common
                                `mappend` inherited
                                `mappend` user
                                )
  ...
  where
    ...
    unsetSymlinkBinDir config =
      ...
      in  config
            { savedInstallFlags = installFlags { installSymlinkBinDir = NoFlag }
            }
...
createPackageEnvironmentFile
  :: Verbosity -> FilePath -> FilePath -> Compiler -> Platform -> IO ()
createPackageEnvironmentFile verbosity sandboxDir pkgEnvFile compiler platform
  = do
    notice verbosity
...
pkgEnvFieldDescrs src
  =  [ simpleField "inherit"
                   (fromFlagOrDefault Disp.empty . fmap Disp.text)
                   (optional parseFilePathQ)
                   pkgEnvInherit
                   (\v pkgEnv -> pkgEnv { pkgEnvInherit = v })
     , commaNewLineListField
       "constraints"
       (Text.disp . fst)
       ((\pc -> (pc, src)) `fmap` Text.parse)
       (configExConstraints . savedConfigureExFlags . pkgEnvSavedConfig)
       ( \v pkgEnv -> updateConfigureExFlags
         pkgEnv
...
parsePackageEnvironment src initial str = do
  ...
  (haddockFlags, installDirs, paths, args) <- foldM
    parseSections
  ...
  where
    ...
    parseSections
      :: SectionsAccum -> ParseUtils.Field -> ParseResult SectionsAccum
    parseSections accum@(h, d, p, a) (ParseUtils.Section _ "haddock" name fs)
      | name == ""
      = do
      ...
      = do
        ...
        return accum
    parseSections (h, d, p, a) (ParseUtils.Section line "install-dirs" name fs)
      | name == ""
      = do
      ...
      ++ userPackageEnvironmentFile
      ++ "' file."
    parseSections accum@(h, d, p, a) (ParseUtils.Section _ "program-locations" name fs)
      | name == ""
      = do
      ...
      = do
        ...
        return accum
    parseSections accum@(h, d, p, a) (ParseUtils.Section _ "program-default-options" name fs)
      | name == ""
      = do
...
showPackageEnvironment pkgEnv =
  showPackageEnvironmentWithComments Nothing pkgEnv
...
showPackageEnvironmentWithComments
  :: (Maybe PackageEnvironment) -> PackageEnvironment -> String
showPackageEnvironmentWithComments mdefPkgEnv pkgEnv =
  Disp.render
    $   ppFields (pkgEnvFieldDescrs ConstraintSourceUnknown) mdefPkgEnv pkgEnv
    $+$ Disp.text ""
    $+$ ppSection "install-dirs"
                  ""
                  installDirsFields
