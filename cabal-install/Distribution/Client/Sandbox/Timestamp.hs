  ...
  case reads timestampString of
    ...
    [] -> case reads timestampString of
      [(timestamps, s)] | all isSpace s -> do
        let
          timestamps' = map
            (\(i, ts) -> (i, map (\(p, t) -> (p, posixSecondsToModTime t)) ts))
            timestamps
...
withTimestampFile
  :: FilePath -> ([TimestampFileRecord] -> IO [TimestampFileRecord]) -> IO ()
...
-- update it.
addTimestamps
  :: ModTime -> [AddSourceTimestamp] -> [FilePath] -> [AddSourceTimestamp]
...
updateTimestamps
  :: [AddSourceTimestamp] -> [FilePath] -> ModTime -> [AddSourceTimestamp]
updateTimestamps timestamps pathsToUpdate newTimestamp = foldr updateTimestamp
                                                               []
                                                               timestamps
  where
    updateTimestamp t@(path, _oldTimestamp) rest
      | path `elem` pathsToUpdate
      = (path, newTimestamp) : rest
      | otherwise
      = t : rest
...
removeTimestamps' l pathsToRemove = foldr removeTimestamp [] l
  where
    removeTimestamp t@(path, _oldTimestamp) rest =
      if path `elem` pathsToRemove then rest else t : rest

-- | If a timestamp record for this compiler doesn't exist, add a new one.
maybeAddCompilerTimestampRecord
  :: Verbosity -> FilePath -> FilePath -> CompilerId -> Platform -> IO ()
maybeAddCompilerTimestampRecord verbosity sandboxDir indexFile compId platform
  = do
    ...
    withTimestampFile sandboxDir $ \timestampRecords -> do
      case lookup key timestampRecords of
        ...
        Nothing -> do
          buildTreeRefs <- listBuildTreeRefs verbosity
                                             ListIgnored
                                             OnlyLinks
                                             indexFile
          now           <- getCurTime
...
withUpdateTimestamps
  ...
  -> IO ()
withUpdateTimestamps = withActionOnCompilerTimestamps updateTimestamps
...
withActionOnAllTimestamps
  :: ([AddSourceTimestamp] -> [FilePath] -> [AddSourceTimestamp])
  -> FilePath
  -> IO [FilePath]
  -> IO ()
...
withActionOnCompilerTimestamps
  :: ([AddSourceTimestamp] -> [FilePath] -> ModTime -> [AddSourceTimestamp])
  -> FilePath
  -> CompilerId
  -> Platform
...
withActionOnCompilerTimestamps f sandboxDir compId platform act = do
  ...
  withTimestampFile sandboxDir $ \timestampRecords -> do
    timestampRecords' <-
      forM timestampRecords $ \r@(key, timestamps) -> if key == needle
        then do
          paths <- act timestamps
          now   <- getCurTime
...
listModifiedDeps verbosity sandboxDir compId platform installedDepsMap = do
  ...
  let needle = timestampRecordKey compId platform
  timestamps <- maybe noTimestampRecord return (lookup needle timestampRecords)
  now        <- getCurTime
  fmap (map fst)
    . filterM (isDepModified verbosity now)
    . filter (\ts -> fst ts `M.member` installedDepsMap)
    $ timestamps
  where
    noTimestampRecord =
      die
        $  "Сouldn't find a timestamp record for the given "
