...
exec
  ...
  -> [String]
  -> IO ()
exec verbosity useSandbox comp platform programDb extraArgs = case extraArgs of
  (exe:args) -> do
    program <- requireProgram' verbosity useSandbox programDb exe
    env     <- ((++) (programOverrideEnv program)) <$> environmentOverrides
    let invocation = programInvocation program { programOverrideEnv = env } args
    runProgramInvocation verbosity invocation

  []         -> die "Please specify an executable to run"
  where
    environmentOverrides = case useSandbox of
      NoSandbox -> return []
...
sandboxEnvironment verbosity sandboxDir comp platform programDb =
  ...
  where
    env getGlobalPackageDB hcProgram packagePathEnvVar = do
      ...
      unless exists
        $  warn verbosity
        $  "Package database is not a directory: "
        ++ sandboxPackagePath
      -- Build the environment
      return
        [ (packagePathEnvVar, Just compilerPackagePaths)
...
requireProgram'
  :: Verbosity -> UseSandbox -> ProgramDb -> String -> IO ConfiguredProgram
requireProgram' verbosity useSandbox programDb exe = do
  (program, _) <- requireProgram verbosity (simpleProgram exe) updateSearchPath
  return program
  where
    updateSearchPath =
      flip modifyProgramSearchPath programDb $ \searchPath -> case useSandbox of
