...
globalCommand commands = CommandUI
  { commandName         = ""
  , commandSynopsis = "Command line interface to the Haskell Cabal infrastructure."
  , commandUsage        = \pname ->
        ...
        ++ unlines
             ...
             ++ if null otherCmds
                then
                  ...
                  par : startGroup "other" : [ addCmd n | n <- otherCmds ]
             )
        ++ "\n"
  ...
  where
    ...
    argsShown
      = [ option ['V']
        ...
        , option
          ...
          (\v flags -> flags { globalSandboxConfigFile = v })
          (reqArgFlag "FILE")
        , option
          []
          ["default-user-config"]
        ...
        , option
          ...
          (\v flags -> flags { globalRequireSandbox = v })
          (boolOpt' ([], ["require-sandbox"]) ([], ["no-require-sandbox"]))
        , option []
                 ["ignore-sandbox"]
                 "Ignore any existing sandbox"
                 globalIgnoreSandbox
                 (\v flags -> flags { globalIgnoreSandbox = v })
                 trueArg
        , option
          []
          ["ignore-expiry"]
    ...
    argsNotShown =
      [ option []
               ...
               ( reqArg' "NAME:URL"
                         (toNubList . maybeToList . readRepo)
      ...
      , option []
               ...
               globalLocalRepos
               (\v flags -> flags { globalLocalRepos = v })
      ...
      , option []
               ...
               globalWorldFile
               (\v flags -> flags { globalWorldFile = v })
               (reqArgFlag "FILE")
      ]
...
configureCommand = c
  ...
  , commandNotes        = Just $ \pname ->
    ( case commandNotes c of
      ...
      ++ "  "
      ++ pname
      ++ " configure --enable-tests -fcustomflag\n"
...
filterConfigureFlags flags cabalLibVersion
  | cabalLibVersion >= Version [1, 23, 0] []
  -- NB: we expect the latest version to be the most common case.
  = flags_latest
  | cabalLibVersion < Version [1, 3, 10] []
  = flags_1_3_10
  | cabalLibVersion < Version [1, 10, 0] []
  = flags_1_10_0
  | cabalLibVersion < Version [1, 12, 0] []
  = flags_1_12_0
  | cabalLibVersion < Version [1, 14, 0] []
  = flags_1_14_0
  | cabalLibVersion < Version [1, 18, 0] []
  = flags_1_18_0
  | cabalLibVersion < Version [1, 19, 1] []
  = flags_1_19_0
  | cabalLibVersion < Version [1, 19, 2] []
  = flags_1_19_1
  | cabalLibVersion < Version [1, 21, 1] []
  = flags_1_20_0
  | cabalLibVersion < Version [1, 22, 0] []
  = flags_1_21_0
  | cabalLibVersion < Version [1, 23, 0] []
  = flags_1_22_0
  | otherwise
  ...
  where
    ...
    flags_1_20_0             = flags_1_21_0
      { configRelocatable = NoFlag
      , configProf        = NoFlag
      , configProfExe     = configProf flags
      , configProfLib     = mappend (configProf flags) (configProfLib flags)
    ...
    flags_1_12_0 =
      flags_1_14_0 { configLibCoverage = NoFlag, configDynExe = NoFlag }
...
configureExCommand = configureCommand
  ...
  , commandOptions      = \showOrParseArgs ->
    liftOptions
        ...
        setFst
        ( filter
            ( (`notElem`["constraint", "dependency", "exact-configuration"])
            . optionName
            )
...
configureExOptions _showOrParseArgs src
  = [ option
      ...
      ( reqArg "VERSION"
               ( readP_to_E ("Cannot parse cabal lib version: "++)
                            (fmap toFlag parse)
               )
    ...
    , option
      ...
      ( reqArg
        "CONSTRAINT"
        (readP_to_E (const "dependency expected") (fmap (\x -> [x]) parse))
        (map display)
      )
...
buildCommand = parent
  ...
  , commandOptions      = \showOrParseArgs ->
    liftOptions fst setFst (commandOptions parent showOrParseArgs)
...
testCommand = parent
  { commandDefaultFlags = (commandDefaultFlags parent, Cabal.defaultBuildFlags, mempty)
  , commandOptions = \showOrParseArgs ->
    liftOptions get1 set1 (commandOptions parent showOrParseArgs)
      ++ liftOptions get2 set2 (Cabal.buildOptions progConf showOrParseArgs)
...
updateCommand = CommandUI
  ...
  , commandDescription  = Just
    $ \_ -> "For all known remote repositories, download the package list.\n"
...
upgradeCommand
  :: CommandUI (ConfigFlags, ConfigExFlags, InstallFlags, HaddockFlags)
...
manpageCommand = CommandUI
  ...
  , commandDescription  = Just $ \_ -> "Output manpage source to STDOUT.\n"
  , commandNotes        = Nothing
...
runCommand = CommandUI
  ...
  , commandDescription  = Just $ \pname ->
    wrapText
      ...
      ++ "Use `"
      ++ pname
      ++ " test --show-details=streaming` to run a "
...
reportCommand = CommandUI
  ...
  , commandDescription  = Nothing
  , commandNotes        = Just
    $ \_ -> "You can store your Hackage login in the ~/.cabal/config file\n"
  , commandUsage        = usageAlternatives "report" ["[FLAGS]"]
  , commandDefaultFlags = defaultReportFlags
  , commandOptions      = \_ ->
    [ optionVerbosity reportVerbosity
                      (\v flags -> flags { reportVerbosity = v })
    , option
      ['u']
      ["username"]
      "Hackage username."
      ...
      (reqArg' "USERNAME" (toFlag . Username) (flagToList . fmap unUsername))
    , option
      ['p']
      ["password"]
      "Hackage password."
      ...
      (reqArg' "PASSWORD" (toFlag . Password) (flagToList . fmap unPassword))
    ]
  }
...
getCommand = CommandUI
  ...
  , commandNotes        = Just $ \pname ->
    "Examples:\n"
      ...
      ++ "  "
      ++ pname
      ++ " get lens --source-repository=head\n"
  ...
  , commandOptions      = \_ ->
    ...
    , option
      ...
      ( optArg
        "[head|this|...]"
        ( readP_to_E (const "invalid source-repository")
...
unpackCommand =
  getCommand { commandName = "unpack", commandUsage = usagePackages "unpack" }
...
listCommand = CommandUI
  ...
  , commandUsage = usageAlternatives "list" ["[FLAGS]", "[FLAGS] STRINGS"]
...
defaultInstallFlags = InstallFlags
  ...
  where
    docIndexFile = toPathTemplate
      ("$datadir" </> "doc" </> "$arch-$os-$compiler" </> "index.html")
...
allSolvers :: String
allSolvers =
  intercalate ", " (map display ([minBound .. maxBound] :: [PreSolver]))
...
installCommand = CommandUI
  ...
  , commandUsage = usageAlternatives "install" ["[FLAGS]", "[FLAGS] PACKAGES"]
  , commandDescription  = Just $ \_ ->
    wrapText
  ...
  , commandNotes        = Just $ \pname ->
    ( case commandNotes $ Cabal.configureCommand defaultProgramConfiguration of
      ...
      ++ "  "
      ++ pname
      ++ " install haddock --bindir=$HOME/hask-bin/ --datadir=$HOME/hask-data/\n"
      ++ "  "
      ++ (map (const ' ') pname)
      ++ "                         "
  ...
  where
    get1 (a, _, _, _) = a
    set1 a (_, b, c, d) = (a, b, c, d)
    get2 (_, b, _, _) = b
    set2 b (a, _, c, d) = (a, b, c, d)
    get3 (_, _, c, _) = c
    set3 c (a, b, _, d) = (a, b, c, d)
    get4 (_, _, _, d) = d
    set4 d (a, b, c, _) = (a, b, c, d)

haddockOptions :: ShowOrParseArgs -> [OptionField HaddockFlags]
haddockOptions showOrParseArgs
  = [ opt
      { optionName  = "haddock-" ++ name
      , optionDescr = [ fmapOptFlags
                        (\(_, lflags) -> ([], map ("haddock-"++) lflags))
                        descr
  ...
  where
    ...
    fmapOptFlags modify (ChoiceOpt xs) =
      ChoiceOpt [ (d, modify f, i, w) | (d, f, i, w) <- xs ]
    fmapOptFlags modify (BoolOpt d f1 f2 r w) =
      BoolOpt d (modify f1) (modify f2) r w
  ...
  [ option ""
    ...
    ++ [ option []
       ...
       , option
         ...
         ( reqArg' "TEMPLATE"
                   (\x -> toNubList [toPathTemplate x])
                   (map fromPathTemplate . fromNubList)
         )
       , option
         []
         ["build-log"]
       ...
       , option []
                ...
                installOneShot
                (\v flags -> flags { installOneShot = v })
                (yesNoOpt showOrParseArgs)
       , option []
                ["run-tests"]
                ...
                trueArg
       , optionNumJobs installNumJobs (\v flags -> flags { installNumJobs = v })
       , option
         []
         ["keep-going"]
...
initCommand = CommandUI
  ...
  , commandOptions      = \_ ->
    [ option ['n']
             ...
             IT.nonInteractive
             (\v flags -> flags { IT.nonInteractive = v })
             trueArg
    , option ['q']
             ["quiet"]
    ...
    , option
      ...
      ( reqArg
        "PACKAGE"
        (readP_to_E ("Cannot parse package name: "++) (toFlag `fmap` parse))
        (flagToList . fmap display)
      )
    , option
      []
      ...
      ( reqArg
        "VERSION"
        (readP_to_E ("Cannot parse package version: "++) (toFlag `fmap` parse))
        (flagToList . fmap display)
      )
    , option
      []
      ...
      ( reqArg
        "VERSION_RANGE"
        ( readP_to_E ("Cannot parse Cabal version range: "++)
                     (toFlag `fmap` parse)
        )
        (flagToList . fmap display)
      )
    , option
      ...
      ( reqArg "LICENSE"
               (readP_to_E ("Cannot parse license: "++) (toFlag `fmap` parse))
               (flagToList . fmap display)
      )
    , option ['a']
             ["author"]
    ...
    , option ['c']
             ...
             ( reqArg' "CATEGORY"
                       (\s -> toFlag $ maybe (Left s) Right (readMaybe s))
    ...
    , option ['x']
             ...
             IT.extraSrc
             (\v flags -> flags { IT.extraSrc = v })
             (reqArg' "FILE" (Just . (:[])) (fromMaybe []))
    , option []
             ["is-library"]
             "Build a library."
             IT.packageType
    ...
    , option
      ...
      ( reqArg
        "LANGUAGE"
        (readP_to_E ("Cannot parse language: "++) (toFlag `fmap` parse))
        (flagToList . fmap display)
      )
    , option
      ['o']
      ...
      ( reqArg
        "MODULE"
        ( readP_to_E ("Cannot parse module name: "++)
                     ((Just . (:[])) `fmap` parse)
        )
        (maybe [] (fmap display))
      )
    , option
      ...
      ( reqArg
        "EXTENSION"
        ( readP_to_E ("Cannot parse extension: "++)
                     ((Just . (:[])) `fmap` parse)
        )
        (maybe [] (fmap display))
      )
    , option
      ...
      "Package dependency."
      IT.dependencies
      (\v flags -> flags { IT.dependencies = v })
      ( reqArg
        "PACKAGE"
        ( readP_to_E ("Cannot parse dependency: "++)
                     ((Just . (:[])) `fmap` parse)
        )
        (maybe [] (fmap display))
      )
    , option []
             ...
             IT.sourceDirs
             (\v flags -> flags { IT.sourceDirs = v })
             (reqArg' "DIR" (Just . (:[])) (fromMaybe []))
    , option []
             ["build-tool"]
             "Required external build tool."
             IT.buildTools
             (\v flags -> flags { IT.buildTools = v })
             (reqArg' "TOOL" (Just . (:[])) (fromMaybe []))
    , optionVerbosity IT.initVerbosity
                      (\v flags -> flags { IT.initVerbosity = v })
    ]
  }
  where
  ...
  where
    ...
    sdistExOptions
      = [ option
          ...
          "archive-format"
          sDistFormat
          (\v flags -> flags { sDistFormat = v })
          ( choiceOpt
            [ (Flag TargzFormat, ([], ["targz"]), "Produce a '.tar.gz' format archive (default and required for uploading to hackage)")
            , (Flag ZipFormat, ([], ["zip"]), "Produce a '.zip' format archive")
            ]
          )
        ]
...
actAsSetupCommand = CommandUI
  ...
  , commandDescription  = Nothing
  , commandNotes        = Nothing
  , commandUsage        = \pname -> "Usage: " ++ pname ++ " act-as-setup\n"
  ...
  , commandOptions      = \_ ->
    [ option
      ...
      ( reqArg "BUILD-TYPE"
               (readP_to_E ("Cannot parse build type: "++) (fmap toFlag parse))
               (map display . flagToList)
      )
    ]
  }
...
sandboxCommand = CommandUI
  ...
  , commandDescription  = Just $ \pname -> concat
    [ paragraph
      $  "Sandboxes are isolated package databases that can be used"
    ...
    , paragraph
      $  "A sandbox in the current directory (created by"
      ...
      ++ " only if called via appropriate "
      ++ pname
      ++ " commands, e.g. `repl`, `build`, `exec`."
    , paragraph
      $  "Currently, "
      ++ pname
      ++ " will not search for a sandbox"
    ...
    , headLine "init:"
    , indentParagraph
      $  "Initialize a sandbox in the current directory."
    ...
    , headLine "delete:"
    , indentParagraph
      $  "Remove the sandbox; deleting all the packages"
      ++ " installed inside."
    , headLine "add-source:"
    , indentParagraph
      $  "Make one or more local packages available in the"
    ...
    , indentParagraph
      ...
      ++ " `"
      ++ pname
      ++ " sandbox hc-pkg unregister` or re-create the"
      ++ " sandbox (`sandbox delete; sandbox init`)."
    , headLine "list-sources:"
    , indentParagraph
      $  "List the directories of local packages made"
      ++ " available via `"
      ++ pname
      ++ " add-source`."
    , headLine "hc-pkg:"
    , indentParagraph
      $  "Similar to `ghc-pkg`, but for the sandbox package"
  ...
  , commandNotes        = Just $ \pname ->
    relevantConfigValuesText ["require-sandbox", "ignore-sandbox"]
      ++ "\n"
      ++ "Examples:\n"
      ...
      ++ "    "
      ++ pname
      ++ " sandbox hc-pkg -- --force unregister broken\n"
  , commandUsage        = usageAlternatives
    "sandbox"
    [ "init          [FLAGS]"
    , "delete        [FLAGS]"
  ...
  , commandOptions      = \_ ->
    ...
    , option
      ...
      sandboxSnapshot
      (\v flags -> flags { sandboxSnapshot = v })
      trueArg
    , option []
             ["sandbox"]
             "Sandbox location (default: './.cabal-sandbox')."
             sandboxLocation
             (\v flags -> flags { sandboxLocation = v })
             (reqArgFlag "DIR")
    ]
  }
...
execCommand = CommandUI
  ...
  , commandDescription  = Just $ \pname ->
    wrapText
      ...
      ++ " GHC uses. `"
      ++ pname
      ++ " exec` can be used to modify this variable:"
...
userConfigCommand = CommandUI
  ...
  , commandDefaultFlags = mempty
  , commandOptions      = \_ ->
    [ optionVerbosity userConfigVerbosity
                      (\v flags -> flags { userConfigVerbosity = v })
    , option ['f']
             ["force"]
...
reqArgFlag
  :: ArgPlaceHolder -> MkOptDescr (b -> Flag String) (Flag String -> b -> b) b
...
yesNoOpt
  :: ShowOrParseArgs -> MkOptDescr (b -> Flag Bool) (Flag Bool -> b -> b) b
yesNoOpt ShowArgs sf lf = trueArg sf lf
yesNoOpt _ sf lf =
  Command.boolOpt' flagToMaybe Flag (sf, lf) ([], map ("no-"++) lf) sf lf

optionSolver
  :: (flags -> Flag PreSolver)
  -> (Flag PreSolver -> flags -> flags)
  -> OptionField flags
optionSolver get set = option
  []
  ["solver"]
  (  "Select dependency solver to use (default: "
  ++ display defaultSolver
  ++ "). Choices: "
  ++ allSolvers
  ++ "."
  )
  ...
  set
  ( reqArg "SOLVER"
           ( readP_to_E (const $ "solver must be one of: " ++ allSolvers)
                        (toFlag `fmap` parse)
           )
           (flagToList . fmap display)
  )
...
usageFlags name pname = "Usage: " ++ pname ++ " " ++ name ++ " [FLAGS]\n"
...
showRepo repo =
  remoteRepoName repo ++ ":" ++ uriToString id (remoteRepoURI repo) []
...
headLine = unlines . map unwords . wrapLine 79 . words

paragraph :: String -> String
paragraph = (++"\n") . unlines . map unwords . wrapLine 79 . words

indentParagraph :: String -> String
indentParagraph =
  unlines . (flip (++)) [""] . map (("  "++) . unwords) . wrapLine 77 . words

relevantConfigValuesText :: [String] -> String
relevantConfigValuesText vs = "Relevant global configuration keys:\n"
  ++ concat [ "  " ++ v ++ "\n" | v <- vs ]
