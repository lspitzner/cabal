...
monitorFileSearchPath notFoundAtPaths foundAtPath =
  monitorFile foundAtPath : map monitorNonExistentFile notFoundAtPaths
...
monitorFileHashedSearchPath notFoundAtPaths foundAtPath =
  monitorFileHashed foundAtPath : map monitorNonExistentFile notFoundAtPaths
...
reconstructMonitorFilePaths (MonitorStateFileSet singlePaths globPaths) =
  ...
  where
    ...
    getGlobPath (MonitorStateGlob kindfile kinddir root gstate) =
      MonitorFileGlob kindfile kinddir $ FilePathGlob root $ case gstate of
...
checkFileMonitorChanged monitor@FileMonitor { fileMonitorKeyValid, fileMonitorCheckIfOnlyValueChanged } root currentKey
  =
    ...
    checkStatusCache (cachedFileStatus, cachedKey, cachedResult) = do
      ...
      where
        ...
        checkForChanges
          ...
          | otherwise
          = checkValueChange cachedKey `mplusMaybeT` checkFileChange
            cachedFileStatus
            cachedKey
            cachedResult
    ...
    checkFileChange cachedFileStatus cachedKey cachedResult = do
      ...
      case res of
        ...
        Right (cachedFileStatus', cacheStatus) -> do
          ...
          whenCacheChanged cacheStatus $ rewriteCacheFile monitor
                                                          cachedFileStatus'
                                                          cachedKey
                                                          cachedResult
...
readCacheFile FileMonitor { fileMonitorCacheFile } =
  withBinaryFile fileMonitorCacheFile ReadMode
    $ \hnd -> Binary.decodeOrFailIO =<< BS.hGetContents hnd
...
rewriteCacheFile FileMonitor { fileMonitorCacheFile } fileset key result =
  writeFileAtomic fileMonitorCacheFile $ Binary.encode (fileset, key, result)
...
probeFileSystem root (MonitorStateFileSet singlePaths globPaths) =
  runChangedM $ do
    ...
    globPaths' <- sequence
      [ probeMonitorStateGlob root globPath | globPath <- globPaths ]
...
mapChangedFile adjust (ChangedM a) = ChangedM (mapStateT (withExceptT adjust) a)
...
probeMonitorStateFileStatus root file status = case status of
  MonitorStateFileExists        -> probeFileExistence root file

  MonitorStateFileModTime mtime -> probeFileModificationTime root file mtime

  MonitorStateFileHashed mtime hash ->
    probeFileModificationTimeAndHash root file mtime hash

  MonitorStateDirExists        -> probeDirExistence root file

  MonitorStateDirModTime mtime -> probeFileModificationTime root file mtime

  MonitorStateNonExistent      -> probeFileNonExistence root file

  MonitorStateAlreadyChanged   -> somethingChanged file
...
probeMonitorStateGlob relroot (MonitorStateGlob kindfile kinddir globroot glob)
  = do
...
probeMonitorStateGlobRel kindfile kinddir root dirName (MonitorStateGlobDirs glob globPath mtime children)
  = do
    ...
    case change of
      Nothing     -> do
        children' <- sequence
          [ do
            fstate' <- probeMonitorStateGlobRel kindfile
                                                kinddir
                                                root
                                                (dirName </> fname)
                                                fstate
  ...
  where
    ...
    probeMergeResult (OnlyInRight path          ) = do
      fstate <- liftIO $ buildMonitorStateGlobRel Nothing
                                                  Map.empty
                                                  kindfile
                                                  kinddir
                                                  root
    ...
    allMatchingFiles dir MonitorStateGlobDirTrailing = [dir]

probeMonitorStateGlobRel _ _ root dirName (MonitorStateGlobFiles glob mtime children)
  = do
    change <- liftIO $ checkDirectoryModificationTime (root </> dirName) mtime
  ...
  where
    probeMergeResult
      :: MergeResult (FilePath, MonitorStateFileStatus) FilePath -> ChangedM ()
...
updateFileMonitor
  ...
  -> IO ()
updateFileMonitor monitor root startTime monitorFiles cachedKey cachedResult =
  do
...
buildMonitorStateFileSet
  :: Maybe MonitorTimestamp -- ^ optional: timestamp
...
buildMonitorStateFileSet mstartTime hashcache root = go Map.empty []
  where
    go
      :: Map FilePath MonitorStateFile
      -> [MonitorStateGlob]
      -> [MonitorFilePath]
      -> IO MonitorStateFileSet
    ...
    go !singlePaths !globPaths (MonitorFile kindfile kinddir path:monitors) =
      do
        monitorState <-
          MonitorStateFile kindfile kinddir
    ...
    go !singlePaths !globPaths (MonitorFileGlob kindfile kinddir globPath:monitors)
      = do
...
buildMonitorStateFile
  :: Maybe MonitorTimestamp -- ^ start time of update
  -> FileHashCache          -- ^ existing file hashes
  -> MonitorKindFile
  -> MonitorKindDir
...
buildMonitorStateFile mstartTime hashcache kindfile kinddir root path = do
  ...
  case (isFile, kindfile, isDir, kinddir) of
    ...
    (False, _, False, _) -> return MonitorStateAlreadyChanged

    (True, FileExists, _, _) -> return MonitorStateFileExists
    ...
    (_, _, True, DirExists) -> return MonitorStateDirExists
...
buildMonitorStateGlob
  ...
  -> IO MonitorStateGlob
buildMonitorStateGlob mstartTime hashcache kindfile kinddir relroot (FilePathGlob globroot globPath)
  = do
    root <- liftIO $ getFilePathRootDirectory globroot relroot
    MonitorStateGlob kindfile kinddir globroot <$> buildMonitorStateGlobRel
      mstartTime
      hashcache
      kindfile
      kinddir
...
buildMonitorStateGlobRel mstartTime hashcache kindfile kinddir root dir globPath
  = do
    let absdir = root </> dir
    ...
    case globPath of
      GlobDir glob globPath' -> do
        ...
        subdirStates <- forM (sort subdirs) $ \subdir -> do
          fstate <- buildMonitorStateGlobRel mstartTime
                                             hashcache
                                             kindfile
                                             kinddir
                                             root
      ...
      GlobFile glob          -> do
        ...
        filesStates <- forM (sort files) $ \file -> do
          fstate <- buildMonitorStateFile mstartTime
                                          hashcache
                                          kindfile
                                          kinddir
                                          root
      ...
      GlobDirTrailing        -> return MonitorStateGlobDirTrailing
...
--
readCacheFileHashes
  :: (Binary a, Binary b) => FileMonitor a b -> IO FileHashCache
readCacheFileHashes monitor =
  handleDoesNotExist Map.empty $ handleErrorCall Map.empty $ do
  ...
  where
    ...
    collectAllGlobHashes globPaths = Map.fromList
      [ (fpath, hash)
...
checkModificationTimeUnchanged root file mtime = handleIOException False $ do
...
readFileHash file = withBinaryFile file ReadMode
  $ \hnd -> evaluate . Hashable.hash =<< BS.hGetContents hnd
...
checkDirectoryModificationTime dir mtime = handleIOException Nothing $ do
  mtime' <- getModTime dir
  if mtime == mtime' then return Nothing else return (Just mtime')

-- | Run an IO computation, returning @e@ if there is an 'error'
-- call. ('ErrorCall')
handleErrorCall :: a -> IO a -> IO a
handleErrorCall e = handle (\(ErrorCall _) -> return e)
...
handleIOException e = handle (anyIOException e)
  where
