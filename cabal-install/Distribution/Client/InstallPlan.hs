...
invariant
  ...
  -> Bool
invariant plan = valid (planIndepGoals plan) (planIndex plan)

-- | Smart constructor that deals with caching the 'Graph' representation.
--
mkInstallPlan
  :: PlanIndex ipkg srcpkg -> IndependentGoals -> GenericInstallPlan ipkg srcpkg
mkInstallPlan index indepGoals = GenericInstallPlan
  { planIndex      = index
...
showPlanIndex
  :: (HasUnitId ipkg, HasUnitId srcpkg) => PlanIndex ipkg srcpkg -> String
showPlanIndex index = intercalate "\n"
...
new
  ...
  -> Either [PlanProblem ipkg srcpkg] (GenericInstallPlan ipkg srcpkg)
new indepGoals index = case problems indepGoals index of
...
remove
  ...
  -> GenericInstallPlan ipkg srcpkg
  -> Either [PlanProblem ipkg srcpkg] (GenericInstallPlan ipkg srcpkg)
remove shouldRemove plan = new (planIndepGoals plan) newIndex
  where
    newIndex = Graph.fromList $ filter (not . shouldRemove) (toList plan)
...
directDeps plan pkgid = case Graph.neighbors (planIndex plan) pkgid of
  Just deps -> deps
...
revDirectDeps
  ...
  -> UnitId
  -> [GenericPlanPackage ipkg srcpkg]
revDirectDeps plan pkgid = case Graph.revNeighbors (planIndex plan) pkgid of
  Just deps -> deps
...
problems _indepGoals index =
  ...
  [ PackageMissingDeps
      pkg
      (catMaybes (map (fmap packageId . flip Graph.lookup index) missingDeps))
    | (pkg, missingDeps) <- Graph.broken index
    ]

    ++ [ PackageCycle cycleGroup | cycleGroup <- Graph.cycles index ]

    ++ [ PackageStateInvalid pkg pkg'
       | pkg       <- Graph.toList index
       , Just pkg' <- map (flip Graph.lookup index) (CD.flatDeps (depends pkg))
...
fromSolverInstallPlan f plan =
  mkInstallPlan (Graph.fromList pkgs') (SolverInstallPlan.planIndepGoals plan)
  where
    (_, pkgs') =
      foldl' f' (Map.empty, []) (SolverInstallPlan.reverseTopologicalOrder plan)
    ...
    f' (pidMap, pkgs) pkg = (pidMap', pkg' : pkgs)
      where
        ...
        pidMap' = case sid of
          PreExistingId _pid uid -> assert (uid == uid') pidMap
          PlannedId pid          -> Map.insert pid uid' pidMap
          where
            sid  = nodeKey pkg
            uid' = nodeKey pkg'
...
configureInstallPlan solverPlan =
  flip fromSolverInstallPlan solverPlan $ \mapDep planpkg -> case planpkg of
    SolverInstallPlan.PreExisting pkg _ -> PreExisting pkg
  ...
  where
    ...
    configureSolverPackage mapDep spkg = ConfiguredPackage
      { confPkgId      = SimpleUnitId $ Configure.computeComponentId
        Cabal.NoFlag
        (packageId spkg)
        PD.CLibName
...
completed plan (Processing processingSet completedSet failedSet) pkgid =
  assert (pkgid `Set.member` processingSet)
    ...
    $ (map asReadyPackage newlyReady, processing')
  where
...
failed plan (Processing processingSet completedSet failedSet) pkgid =
  assert (pkgid `Set.member` processingSet)
    ...
    $ assert (processingInvariant plan processing')
    $ (map asConfiguredPackage (tail newlyFailed), processing')
  where
    ...
    newlyFailed =
      fromMaybe (internalError "package not in graph")
...
processingInvariant plan (Processing processingSet completedSet failedSet) =
  all (isJust . flip Graph.lookup (planIndex plan)) (Set.toList processingSet)
    && all (isJust . flip Graph.lookup (planIndex plan))
           (Set.toList completedSet)
...
executionOrder plan =
  let (newpkgs, processing) = ready plan in tryNewTasks processing newpkgs
  where
    ...
    waitForTasks !results tasksFailed !processing = do
      ...
      case result of
        ...
        Right _success -> do
          ...
          tryNewTasks results' tasksFailed tasksRemaining processing' nextpkgs
          where
        ...
        Left _failure -> do
          ...
          when (not tasksFailed && not keepGoing) $ cancelJobs jobCtl
