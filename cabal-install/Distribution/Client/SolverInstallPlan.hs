...
mkInstallPlan :: SolverPlanIndex -> IndependentGoals -> SolverInstallPlan
mkInstallPlan index indepGoals = SolverInstallPlan
  { planIndex      = index
...
showPlanProblem (PackageInconsistency name inconsistencies) =
  "Package "
    ...
    ++ unlines
         [ "  package " ++ display pkg ++ " requires " ++ display
...
problems indepGoals index =
  ...
  [ PackageMissingDeps
    ...
    ++ [ PackageInconsistency name inconsistencies
       | (name, inconsistencies) <- dependencyInconsistencies indepGoals index
       ]

    ++ [ PackageStateInvalid pkg pkg'
       | pkg       <- Graph.toList index
       , Just pkg' <- map (flip Graph.lookup index) (nodeNeighbors pkg)
...
dependencyInconsistencies indepGoals index = concatMap
  ...
  where
    ...
    subplans = -- Not Graph.closure!!
               map (nonSetupClosure index) (rootSets indepGoals index)
...
nonSetupClosure index pkgids0 = closure Graph.empty pkgids0
  where
    ...
    closure completed (pkgid:pkgids) = case Graph.lookup pkgid index of
      Nothing  -> closure completed pkgids
      Just pkg -> case Graph.lookup (nodeKey pkg) completed of
...
rootSets (IndependentGoals indepGoals) index = if indepGoals
  then map (:[]) libRoots
  else [libRoots] ++ setupRoots index
  where
...
libraryRoots index = map (nodeKey . toPkgId) roots
  where
...
setupRoots =
  filter (not . null) . map (CD.setupDeps . resolverPackageDeps) . Graph.toList
...
--
dependencyInconsistencies'
  :: SolverPlanIndex -> [(PackageName, [(PackageIdentifier, Version)])]
dependencyInconsistencies' index =
  ...
  where
    ...
    inverseIndex
      :: Map PackageName (Map SolverId (SolverPlanPackage, [PackageId]))
    ...
    reallyIsInconsistent [p1, p2] =
      let pid1 = nodeKey p1
          pid2 = nodeKey p2
      in  pid1
            `notElem` CD.nonSetupDeps (resolverPackageDeps p2)
            &&        pid2
            `notElem` CD.nonSetupDeps (resolverPackageDeps p1)
