...
downloadURI transport  verbosity uri path                            = do
  ...
  withTempFileName (takeDirectory path) (takeFileName path) $ \tmpFile -> do
    ...
    case fst result of
      ...
      errCode ->
        die $ "Failed to download " ++ show uri ++ " : HTTP code " ++ show
          errCode
...
transportCheckHttps transport uri
  | uriScheme uri == "https:"
  , not (transportSupportsHttps transport)
  = die $ "The URL " ++ show uri ++ " " ++ requiresHttpsErrorMessage
  | otherwise
  = return ()
...
remoteRepoTryUpgradeToHttps transport repo
  ...
  = return repo { remoteRepoURI = (remoteRepoURI repo) { uriScheme = "https:" }
                }
  | otherwise
  = return repo
...
isOldHackageURI uri = case uriAuthority uri of
...
noPostYet
  :: Verbosity -> URI -> String -> Maybe (String, String) -> IO (Int, String)
...
supportedTransports =
  [ let prog = simpleProgram "curl"
    in  ("curl", Just prog, True, \db -> curlTransport <$> lookupProgram prog db)
  , let prog = simpleProgram "wget"
    in  ("wget", Just prog, True, \db -> wgetTransport <$> lookupProgram prog db)
  , let prog = simpleProgram "powershell"
    in  ("powershell", Just prog, True, \db ->
          powershellTransport <$> lookupProgram prog db)
  , ("plain-http", Nothing, False, \_ -> Just plainHttpTransport)
  ]
  ...
  case find (\(name', _, _, _) -> name' == name) supportedTransports of
    ...
    Nothing ->
      die
        ...
        ++ intercalate ", " [ name' | (name', _, _, _) <- supportedTransports ]
...
configureTransport verbosity Nothing = do
  ...
  progdb <- configureAllKnownPrograms verbosity $ addKnownPrograms
...
curlTransport prog = HttpTransport gethttp
                                   posthttp
                                   posthttpfile
                                   puthttpfile
                                   True
  ...
  where
    gethttp verbosity uri etag destPath reqHeaders = do
      withTempFile (takeDirectory destPath) "curl-headers.txt"
        $ \tmpFile tmpHandle -> do
            hClose tmpHandle
            let
              args =
    ...
    addAuthConfig auth progInvocation = progInvocation
      { progInvokeInput = do
        (uname, passwd) <- auth
        return $ unlines ["--digest", "--user " ++ uname ++ ":" ++ passwd]
    ...
    posthttpfile verbosity uri path auth = do
      ...
      resp               <- getProgramInvocationOutput verbosity
        $ addAuthConfig auth (programInvocation prog args)
    ...
    parseResponse uri resp headers =
      let
        codeerr  = case reverse (lines resp) of
          (codeLine:rerrLines) -> case readMaybe (trim codeLine) of
            Just i  -> let errstr = mkErrstr rerrLines in Just (i, errstr)
            Nothing -> Nothing
          []                   -> Nothing
        ...
        mb_etag
          = listToMaybe
          $ reverse [ etag | ["ETag:", etag] <- map words (lines headers) ]
      in
        case codeerr of
...
wgetTransport prog = HttpTransport gethttp
                                   posthttp
                                   posthttpfile
                                   puthttpfile
                                   True
  ...
  where
    ...
    posthttpfile verbosity uri path auth =
      withTempFile (takeDirectory path) (takeFileName path)
        $ \tmpFile tmpHandle ->
    ...
    runWGet verbosity uri args = do
      ...
      let invocation = (programInvocation prog ("--input-file=-" : args))
      ...
      then
        ...
        die $ "'" ++ programPath prog ++ "' exited with an error:\n" ++ resp
...
powershellTransport prog = HttpTransport gethttp
                                         posthttp
                                         posthttpfile
                                         puthttpfile
                                         True
  ...
  where
    ...
    parseUploadResponse uri resp = case lines (trim resp) of
      (codeStr:message) | Just code <- readMaybe codeStr ->
        return (code, unlines message)
      _ -> statusParseFail uri resp
...
plainHttpTransport = HttpTransport gethttp
                                   posthttp
                                   posthttpfile
                                   puthttpfile
                                   False
  ...
  where
    gethttp verbosity uri etag destPath reqHeaders = do
      let req = Request
            { rqURI     = uri
            , rqMethod  = GET
            , rqHeaders = [ Header HdrIfNoneMatch t | t <- maybeToList etag ]
              ++ reqHeaders
            , rqBody    = BS.empty
            }
      ...
      when (code == 200 || code == 206) $ writeFileAtomic destPath $ rspBody
        resp
      return (code, etag')
    ...
    rspErrorString resp = case lookupHeader HdrContentType (rspHeaders resp) of
      Just contenttype | takeWhile (/=';') contenttype == "text/plain" -> BS.unpack
        (rspBody resp)
      _ -> rspReason resp

    cabalBrowse verbosity auth act = do
      p <- fixupEmptyProxy <$> fetchProxy True
      Exception.handleJust (guard . isDoesNotExistError)
                           (  const
                           .  die
  ...
  where
    formatBody content boundary =
      BS.concat
        ...
        ++ [crlf, content, crlf, dd, boundary, dd, crlf]
...
getProgramInvocationOutputAndErrors verbosity ProgramInvocation { progInvokePath = path, progInvokeArgs = args, progInvokeEnv = envOverrides, progInvokeCwd = mcwd, progInvokeInput = minputStr, progInvokeOutputEncoding = encoding }
  = do
    let utf8 = case encoding of
          IOEncodingUTF8 -> True
          _              -> False
        decode
          | utf8
          = fromUTF8 . normaliseLineEndings
          | otherwise
          = id
  ...
  where
    input = case minputStr of
      Nothing       -> Nothing
      Just inputStr -> Just $ case encoding of
