  ...
  where
    ...
    useFirstError = replace Nothing
      where
        ...
        replace Nothing (Step x@(Failure cs Backjump) xs) =
          Step x $ replace (Just cs) xs
    ...
    go ms _ (Fail (exh, Just cs)) =
      Fail
        $  "Could not resolve dependencies:\n"
        ++ unlines
             ( messages $ showMessages
               (L.foldr (\v _ -> v `CS.member` cs) True)
               False
               ms
             )
        ++ ( if exh
      ...
      where
        currlimit (Just n) = "currently " ++ show n ++ ", "
        currlimit Nothing  = ""
    go _ _ (Done s) = Done s
    go _ _ (Fail (_, Nothing)) =
      Fail ("Could not resolve dependencies; something strange happened.") -- should not happen
...
tryWith :: Message -> Progress Message fail done -> Progress Message fail done
