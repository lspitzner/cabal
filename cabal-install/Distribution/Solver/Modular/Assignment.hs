...
extend
  :: (Extension -> Bool) -- ^ is a given extension supported
  ...
  where
    ...
    extendSingle a (Dep qpn ci) =
      ...
      in  case (\x -> M.insert qpn x a) <$> merge ci' ci of
            Left (c, (d, d')) ->
              Left (c, L.map (Dep qpn) (simplify (P qpn) d d'))
            Right x -> Right x
...
toCPs (A pa fa sa) rdm =
  let
    -- get hold of the graph
      ...
      (g, vm, cvm) =
        graphFromEdges (L.map (\(x, xs) -> ((), x, nub xs)) (M.toList rdm))
      tg :: Graph Component
      tg = transposeG g
      ...
      ps :: [PI QPN]
      ps = L.map ((\(_, x, _) -> PI x (pa M.! x)) . vm) $ topSort g
