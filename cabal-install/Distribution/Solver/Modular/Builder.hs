...
extendOpen qpn' gs s@(BS { rdeps = gs', open = o' }) = go gs' o' gs
  where
    go
      :: RevDepMap -> PSQ (OpenGoal ()) () -> [OpenGoal Component] -> BuildState
    ...
    go g o (ng@(OpenGoal (Simple (Dep qpn _) c) _gr):ngs)
      ...
      = go g o ngs
      | qpn `M.member` g
      = go (M.adjust ((c, qpn'):) qpn g) o ngs
      | otherwise
      = go (M.insert qpn [(c, qpn')] g) (cons' ng () o) ngs
...
scopedExtendOpen qpn i gr fdeps fdefs s = extendOpen qpn gs s
  where
    ...
    qfdefs =
      L.map (\(fn, b) -> Flagged (FN (PI qpn i) fn) b [] []) $ M.toList fdefs
      ...
      = FChoiceF
        ...
        ( P.fromList
          ( reorder
            ...
            [ (True, ( extendOpen
                       qpn
                       (L.map (flip OpenGoal (FDependency qfn True)) t)
                       bs
                     )
              { next = Goals
              })
            , (False, ( extendOpen
                        qpn
                        (L.map (flip OpenGoal (FDependency qfn False)) f)
                        bs
                      )
              { next = Goals
              })
    ...
    go bs@(BS { next = OneGoal (OpenGoal (Stanza qsn@(SN (PI qpn _) _) t) gr) })
      = SChoiceF
        ...
        ( P.fromList
          [ (False, bs { next = Goals })
          , (True, ( extendOpen qpn
                                (L.map (flip OpenGoal (SDependency qsn)) t)
                                bs
                   )
            { next = Goals
            })
...
buildTree idx (IndependentGoals ind) igs = build BS
  { index          = idx
  ...
  where
    ...
    qpns
      ...
      = makeIndependent igs
      | otherwise
      = L.map (Q (PackagePath DefaultNamespace Unqualified)) igs
